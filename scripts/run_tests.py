...
IGNORE_PREFS_FILE_IN = os.path.join(
    'src', 'enso', 'tests', 'ignorePrefs.json'
)
...
def copy_ignore_prefs():
  print("Copying ignorePrefs.json from src/enso/tests to engine/testing/mochitest...")
...
def main():
  ...
  if path in ("", "all"):
    test_dirs = [p for p in Path("enso/tests").iterdir() if p.is_dir() and p.name != MOCHITEST_NAME]
    mochitest_dirs = [p for p in Path(f"enso/tests/{MOCHITEST_NAME}").iterdir() if p.is_dir()]
    test_dirs.extend(mochitest_dirs)
    test_paths = [str(p) for p in test_dirs]
    run_mach_with_paths(test_paths)
  else:
    run_mach_with_paths([f"enso/tests/{path}"])
