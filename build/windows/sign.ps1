...
Start-Job -Name "DownloadGitl10n" -ScriptBlock {
    param($PWD)
    cd $PWD
    $env:ENSO_L10N_CURR_DIR=[regex]::replace($PWD, "^([A-Z]):", { "/" + $args.value.Substring(0, 1).toLower() }) -replace "\\", "/"
...
function SignAndPackage($name) {
    ...
    $env:ENSO_SETUP_EXE_PATH="$PWD\windsign-temp\windows-x64-obj-$name\browser\installer\windows\instgen\setup.exe"
    ...
    if ($name -eq "arm64") {
        mv .\dist\enso.installer.exe windsign-temp\windows-x64-signed-$name\enso.installer-$name.exe
    } else {
        mv .\dist\enso.installer.exe windsign-temp\windows-x64-signed-$name\enso.installer.exe
    }
