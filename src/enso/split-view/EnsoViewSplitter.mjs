...
class nsEnsoViewSplitter extends nsEnsoDOMOperatedFeature {
  currentView = -1;
  _data = [];
  ...
  init() {
    ...
    XPCOMUtils.defineLazyPreferenceGetter(
      this,
      "_edgeHoverSize",
      "enso.splitView.rearrange-edge-hover-size",
      24
    );

    ChromeUtils.defineLazyGetter(this, "overlay", () =>
      document.getElementById("enso-splitview-overlay")
    );

    ChromeUtils.defineLazyGetter(this, "dropZone", () =>
      document.getElementById("enso-splitview-dropzone")
    );
    ...
    if (Services.prefs.getBoolPref("enso.splitView.enable-tab-drop")) {
  ...
  insertIntoContextMenu() {
    ...
    menuitem.setAttribute("id", "context-ensoSplitLink");
    menuitem.setAttribute("hidden", "true");
    menuitem.setAttribute("command", "cmd_ensoSplitViewLinkInNewTab");
    menuitem.setAttribute("data-l10n-id", "enso-split-link");
  ...
  onTabSelect(event) {
    ...
    if (previousTab && !previousTab.hasAttribute("enso-empty-tab")) {
  ...
  removeTabFromGroup(
    ...
    if (tab.group && tab.group.hasAttribute("split-view-group")) {
      ...
      this.#dispatchItemEvent("EnsoTabRemovedFromSplit", tab);
    }
    if (group.tabs.length < 2) {
      ...
      if (changeTab) {
        ...
        document.getElementById("cmd_ensoNewEmptySplit").removeAttribute("disabled");
      }
    } else {
  ...
  #getDragImageForSplit(tab) {
    const element = window.MozXULElement.parseXULToFragment(
      `
      <vbox id="enso-split-view-drag-image">
        <image />
        <label />
      </vbox>
    `
    ).querySelector("#enso-split-view-drag-image");
  ...
  _calculateDropSide(event, panelsRect) {
    ...
    // `enso.splitView.tab-drop-treshold`
  ...
  onBrowserDragOverToSplit(event) {
    ...
    if (dt.mozTypesAt(0)[0] == TAB_DROP_TYPE) {
      ...
      if (
        ...
        gBrowser.selectedTab.hasAttribute("enso-empty-tab") ||
    ...
    if (
      !this._lastOpenedTab ||
      (this._lastOpenedTab.getAttribute("enso-workspace-id") !==
        draggedTab.getAttribute("enso-workspace-id") &&
        !this._lastOpenedTab.hasAttribute("enso-essential") &&
        !draggedTab.hasAttribute("enso-essential"))
    ) {
  ...
  _animateDropEdge(dropSide, currentView, draggedTab, oldTab) {
    ...
    const padding = EnsoThemeModifier.elementSeparation;
    this.fakeBrowser.setAttribute("flex", "1");
    this.fakeBrowser.id = "enso-split-view-fake-browser";
    if (oldTab.splitView) {
    ...
    this._finishAllAnimatingPromise = Promise.all([
      gEnsoUIManager.motion.animate(gBrowser.tabbox, animateTabBox, {
        duration: 0.1,
        easing: "ease-out",
      }),
      gEnsoUIManager.motion.animate(this.fakeBrowser, animateFakeBrowser, {
        duration: 0.1,
        easing: "ease-out",
      }),
    ]);
  ...
  afterRearangeAction() {
    document.getElementById("ensoSplitViewModifier").hidePopup();
    gEnsoUIManager.showToast("enso-split-view-modifier-enabled-toast", {
      descriptionId: "enso-split-view-modifier-enabled-toast-description",
    });
  }

  afterRearangeRemove() {
    gEnsoUIManager.showToast("enso-split-view-modifier-disabled-toast");
  }
  ...
  disableTabRearrangeView = (event = null) => {
    ...
    if (
      ...
      (event && event.target.classList.contains("enso-split-view-splitter"))
    ) {
      return;
    }
  ...
  onBrowserDragStart = (event) => {
    ...
    const container = event.target.closest(".browserSidebarContainer[enso-split]");
    if (container && event.target.closest(".enso-tab-rearrange-button")) {
  ...
  resetTabState(tab, forUnsplit) {
    ...
    tab.linkedBrowser.ensoModeActive = false;
    const container = tab.linkedBrowser.closest(".browserSidebarContainer");
    container.removeAttribute("is-enso-split");
  ...
  insetUpdateContextMenuItems() {
    ...
    contentAreaContextMenu.addEventListener("popupshowing", () => {
      ...
      document.getElementById("context_ensoSplitTabs").setAttribute("data-l10n-args", tabCountInfo);
      const splitTabs = document.getElementById("context_ensoSplitTabs");
  ...
  insertSplitViewTabContextMenu() {
    const element = window.MozXULElement.parseXULToFragment(`
      <menuitem id="context_ensoSplitTabs"
                data-lazy-l10n-id="tab-enso-split-tabs"
                data-l10n-args='{"tabCount": 1}'
                command="cmd_ensoSplitViewContextMenu"/>
    `);
  ...
  splitLinkInNewTab() {
    ...
    const currentTab = gEnsoGlanceManager.getTabOrGlanceParent(window.gBrowser.selectedTab);
  ...
  contextCanSplitTabs() {
    ...
    for (const tab of window.gBrowser.selectedTabs) {
      if (tab.splitView || tab.hasAttribute("enso-empty-tab")) {
        return false;
      }
    }
    return true;
  ...
  onLocationChange(browser) {
    ...
    const ignoreSplit = tab.hasAttribute("enso-dont-split-glance");
    tab.removeAttribute("enso-dont-split-glance");
    let isGlanceTab = false;
    if (tab.hasAttribute("enso-glance-tab") && !ignoreSplit) {
      // Extract from parent node so we are not selecting the wrong (current) tab
      tab = tab.parentNode.closest(".tabbrowser-tab");
      isGlanceTab = true;
      if (!tab) {
        console.error("Tab not found for enso-glance-tab");
      }
    }
    if (tab) {
    ...
    this._maybeRemoveFakeBrowser();
    {
      const shouldDisableEmptySplits = tab.hasAttribute("enso-empty-tab") || tab.splitView;
      const command = document.getElementById("cmd_ensoNewEmptySplit");
  ...
  _moveTabsToContainer(tabs, relativeTab) {
    ...
    const relativeTabIsEssential = relativeTab.hasAttribute("enso-essential");

    if (relativeTabIsEssential) {
      gEnsoPinnedTabManager.addToEssentials(tabs);
    } else {
  ...
  splitTabs(tabs, gridType, initialIndex = 0, { groupFetchId = null } = {}) {
    ...
    return this.#withoutSplitViewTransition(() => {
      ...
      tabs = tabs.filter((t) => !t.hidden && !t.hasAttribute("enso-empty-tab"));
      ...
      const thereIsOneEssential = tabs.some((tab) => tab.hasAttribute("enso-essential"));
  ...
  deactivateCurrentSplitView({ removeDeckSelected = false } = {}) {
    ...
    this.removeSplitters();
    this.tabBrowserPanel.removeAttribute("enso-split-view");
    document.getElementById("tabbrowser-tabbox").removeAttribute("enso-split-view");
    this.currentView = -1;
    this.toggleWrapperDisplay(false);
    this.maybeDisableOpeningTabOnSplitView();
    window.dispatchEvent(new CustomEvent("EnsoViewSplitter:SplitViewDeactivated"));
  }
  ...
  activateSplitView(splitData, reset = false) {
    ...
    // Apply grid to tabs first to set enso-split attribute on containers
    // before setting enso-split-view on parents. This prevents the black flash
    // caused by CSS rules that hide containers without enso-split attribute
    // when the parent has enso-split-view attribute.
    this.applyGridToTabs(splitData.tabs);

    this.tabBrowserPanel.setAttribute("enso-split-view", "true");
    document.getElementById("tabbrowser-tabbox").setAttribute("enso-split-view", "true");

    this.applyGridLayout(splitData.layoutTree);
    this.setTabsDocShellState(splitData.tabs, true);
    this.toggleWrapperDisplay(true);
    window.dispatchEvent(new CustomEvent("EnsoViewSplitter:SplitViewActivated"));
  }
  ...
  applyGridToTabs(tabs) {
    tabs.forEach((tab) => {
      ...
      const container = tab.linkedBrowser?.closest(".browserSidebarContainer");
      container.setAttribute("is-enso-split", "true");
      // Set enso-split early to prevent visibility flash when switching workspaces.
      // The CSS rule for [is-enso-split]:not([enso-split]) sets visibility:hidden,
      // so we must set enso-split before the parent enso-split-view attribute is applied.
      container.setAttribute("enso-split", "true");
      if (!container?.querySelector(".enso-tab-rearrange-button")) {
  ...
  _createHeader(container) {
    ...
    headerContainer.classList.add("enso-view-splitter-header-container");
    const header = document.createElement("div");
    header.classList.add("enso-view-splitter-header");
    const removeButton = document.createXULElement("toolbarbutton");
    removeButton.classList.add("enso-tab-unsplit-button");
    removeButton.addEventListener("click", () => {
      this.removeTabFromSplit(container);
    });
    const rearrangeButton = document.createXULElement("toolbarbutton");
    rearrangeButton.classList.add("enso-tab-rearrange-button");
  ...
  _removeHeader(container) {
    const header = container.querySelector(".enso-view-splitter-header-container");
    if (header) {
      header.remove();
    }
  }
  ...
  createSplitter(orient, parentNode, idx) {
    ...
    splitter.className = "enso-split-view-splitter";
  ...
  removeSplitters() {
    [...this.overlay.children]
      .filter((c) => c.classList.contains("enso-split-view-splitter"))
      .forEach((s) => s.remove());
  ...
  handleSplitterMouseDown = (event) => {
    this.tabBrowserPanel.setAttribute("enso-split-resizing", true);
    ...
    document.addEventListener(
      ...
      () => {
        ...
        this.tabBrowserPanel.removeAttribute("enso-split-resizing");
      },
      { once: true }
    );
  };
  ...
  setTabsDocShellState(tabs, active) {
    for (const tab of tabs) {
      // ensoModeActive allow us to avoid setting docShellisActive to false later on,
      // see browser-custom-elements.js's patch
      tab.linkedBrowser.ensoModeActive = active;
  ...
  resetContainerStyle(container, removeDeckSelected = false) {
    container.removeAttribute("enso-split");
  ...
  updatePanelUI(panel) {
    for (const gridType of ["hsep", "vsep", "grid", "unsplit"]) {
      const selector = panel.querySelector(`.enso-split-view-modifier-preview.${gridType}`);
  ...
  moveTabToSplitView(event, draggedTab) {
    ...
    let droppedOnTab = gEnsoGlanceManager.getTabOrGlanceParent(gBrowser.getTabForBrowser(browser));
    ...
    if (droppedOnTab && droppedOnTab !== draggedTab) {
      ...
      const thereIsOneEssential = tempTabs.some((tab) => tab.hasAttribute("enso-essential"));
  ...
  _getSplitViewGroup(tabs, id = null) {
    if (tabs.some((tab) => tab.hasAttribute("enso-essential"))) {
      return null;
    }
  ...
  maybeDisableOpeningTabOnSplitView() {
    ...
    document
      .getElementById("cmd_ensoSplitViewLinkInNewTab")
      .setAttribute("disabled", shouldBeDisabled);
    const splitGlanceCommand = document.getElementById("cmd_ensoGlanceSplit");
    if (shouldBeDisabled) {
  ...
  #withoutSplitViewTransition(callback) {
    this.tabBrowserPanel.classList.add("enso-split-view-no-transition");
    try {
      return callback();
    } finally {
      requestAnimationFrame(() => {
        this.tabBrowserPanel.classList.remove("enso-split-view-no-transition");
      }, 0);
    }
  }
  ...
  createEmptySplit(side = "right") {
    ...
    const emptyTab = gEnsoWorkspaces._emptyTab;
    ...
    this.#withoutSplitViewTransition(() => {
      ...
      setTimeout(() => {
          ...
          (event) => {
            ...
            if (onElementPicked) {
              if (
                ...
                selectedTab.getAttribute("enso-workspace-id") !==
                  newSelectedTab.getAttribute("enso-workspace-id")
              ) {
                cleanup();
                return;
              }
        ...
        gEnsoUIManager.handleNewTab(false, false, "tab", true);
      });
    });
  }
...
window.gEnsoViewSplitter = new nsEnsoViewSplitter();
