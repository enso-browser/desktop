...
import { nsEnsoDOMOperatedFeature } from "chrome://browser/content/enso-components/EnsoCommonUtils.mjs";
...
class ZenPinnedTabsObserver {
  ...
  constructor() {
    ...
    XPCOMUtils.defineLazyPreferenceGetter(
      lazy,
      "ensoPinnedTabRestorePinnedTabsToPinnedUrl",
      "enso.pinned-tab-manager.restore-pinned-tabs-to-pinned-url",
      false
    );
    XPCOMUtils.defineLazyPreferenceGetter(
      lazy,
      "ensoPinnedTabCloseShortcutBehavior",
      "enso.pinned-tab-manager.close-shortcut-behavior",
      "switch"
    );
    XPCOMUtils.defineLazyPreferenceGetter(
      lazy,
      "ensoTabsEssentialsMax",
      "enso.tabs.essentials.max",
      12
    );
...
class nsEnsoPinnedTabManager extends nsEnsoDOMOperatedFeature {
  init() {
    if (!this.enabled) {
      return;
    }
    this._canLog = Services.prefs.getBoolPref("enso.pinned-tab-manager.debug", false);
    ...
    this._ensoClickEventListener = this._onTabClick.bind(this);

    gEnsoWorkspaces._resolvePinnedInitialized();
    if (lazy.ensoPinnedTabRestorePinnedTabsToPinnedUrl) {
      gEnsoWorkspaces.promiseInitialized.then(() => {
        for (const tab of gEnsoWorkspaces.allStoredTabs) {
          try {
  ...
  log(message) {
    if (this._canLog) {
      ...
      console.log(`[EnsoPinnedTabManager] ${message}`);
    }
  }
  ...
  setEssentialTabIcon(tab, url = null) {
    ...
    tab.style.setProperty("--enso-essential-tab-icon", `url(${iconUrl})`);
  }
  ...
  get enabled() {
    return !gEnsoWorkspaces.privateWindowOrDisabled;
  }

  get maxEssentialTabs() {
    return lazy.ensoTabsEssentialsMax;
  }
  ...
  _onPinnedTabEvent(action, event) {
    ...
    switch (action) {
      case "TabPinned":
        tab._ensoClickEventListener = this._ensoClickEventListener;
        tab.addEventListener("click", tab._ensoClickEventListener);
        break;
      // [Fall through]
      case "TabUnpinned":
        if (tab._ensoClickEventListener) {
          tab.removeEventListener("click", tab._ensoClickEventListener);
          delete tab._ensoClickEventListener;
        }
        this.resetPinChangedUrl(tab);
        break;
      default:
        console.warn("EnsoPinnedTabManager: Unhandled tab event", action);
        break;
    }
  }
  ...
  async _onTabClick(e) {
    ...
    if (e.button === 1 && tab) {
      await this.onCloseTabShortcut(e, tab, {
        closeIfPending: Services.prefs.getBoolPref("enso.pinned-tab-manager.wheel-close-if-pending"),
      });
    }
  }
  ...
  replacePinnedUrlWithCurrent(tab = undefined) {
    ...
    window.gEnsoWindowSync.setPinnedTabState(tab);
    this.resetPinChangedUrl(tab);
    gEnsoUIManager.showToast("enso-pinned-tab-replaced");
  }
  ...
  async onCloseTabShortcut(
    ...
    {
      behavior = lazy.ensoPinnedTabCloseShortcutBehavior,
      noClose = false,
          ...
          if (behavior.includes("unload")) {
            for (const tab of pinnedTabs) {
              if (tab.hasAttribute("glance-id")) {
                ...
                // it as well.
                gEnsoGlanceManager.manageTabClose(tab.glanceTab);
              ...
              if (!folderToUnload && tab.hasAttribute("folder-active")) {
                await gEnsoFolders.animateUnload(group, tab);
              }
            }
            if (folderToUnload) {
              await gEnsoFolders.animateUnloadAll(folderToUnload);
            }
            const allAreUnloaded = pinnedTabs.every(
              (tab) => tab.hasAttribute("pending") && !tab.hasAttribute("enso-essential")
            );
  ...
  _handleTabSwitch(selectedTab) {
    ...
    if (!nextTab) {
      gEnsoWorkspaces.selectEmptyTab();
      return;
    }
  ...
  _resetTabToStoredState(tab) {
    ...
    const initialState = tab._ensoPinnedInitialState;
    ...
    state.image = tab.ensoStaticIcon || initialState.image;
    state.index = 0;
  ...
  addToEssentials(tab) {
    ...
    for (let i = 0; i < tabs.length; i++) {
      ...
      let tab = tabs[i];
      const section = gEnsoWorkspaces.getEssentialsSection(tab);
      ...
      if (tab.hasAttribute("enso-workspace-id")) {
        tab.removeAttribute("enso-workspace-id");
      }
      if (tab.pinned) {
      ...
      if (tab.selected) {
        gEnsoWorkspaces.switchTabIfNeeded(tab);
      }
  ...
  removeEssentials(tab, unpin = true) {
    ...
    for (let i = 0; i < tabs.length; i++) {
      // eslint-disable-next-line no-shadow
      const tab = tabs[i];
      tab.removeAttribute("enso-essential");
      if (gEnsoWorkspaces.workspaceEnabled && gEnsoWorkspaces.getActiveWorkspaceFromCache().uuid) {
        tab.setAttribute("enso-workspace-id", gEnsoWorkspaces.getActiveWorkspaceFromCache().uuid);
      }
      if (unpin) {
        gBrowser.unpinTab(tab);
      } else {
        gBrowser.ensoHandleTabMove(tab, () => {
          const pinContainer = gEnsoWorkspaces.pinnedTabsContainer;
  ...
  _insertItemsIntoTabContextMenu() {
    ...
    const elements = window.MozXULElement.parseXULToFragment(`
            <menuseparator id="context_enso-pinned-tab-separator" hidden="true"/>
            <menuitem id="context_enso-replace-pinned-url-with-current"
                      data-lazy-l10n-id="tab-context-enso-replace-pinned-url-with-current"
                      hidden="true"
                      command="cmd_ensoReplacePinnedUrlWithCurrent"/>
            <menuitem id="context_enso-reset-pinned-tab"
                      data-lazy-l10n-id="tab-context-enso-reset-pinned-tab"
                      hidden="true"
                      command="cmd_ensoPinnedTabResetNoTab"/>
        `);
    document.getElementById("tabContextMenu").appendChild(elements);

    const element = window.MozXULElement.parseXULToFragment(`
            <menuitem id="context_enso-add-essential"
                      data-l10n-id="tab-context-enso-add-essential"
                      hidden="true"
                      disabled="true"
                      command="cmd_contextZenAddToEssentials"/>
            <menuitem id="context_enso-remove-essential"
                      data-lazy-l10n-id="tab-context-enso-remove-essential"
                      hidden="true"
                      command="cmd_contextZenRemoveFromEssentials"/>
            <menuseparator/>
            <menuitem id="context_enso-edit-tab-title"
                      data-lazy-l10n-id="tab-context-enso-edit-title"
                      hidden="true"/>
            <menuitem id="context_enso-edit-tab-icon"
                      data-lazy-l10n-id="tab-context-enso-edit-icon"/>
            <menuseparator/>
        `);

    document.getElementById("context_pinTab")?.before(element);
    document.getElementById("context_enso-edit-tab-title").addEventListener("command", (event) => {
      gEnsoVerticalTabsManager.renameTabStart(event);
    });
    document.getElementById("context_enso-edit-tab-icon").addEventListener("command", () => {
      const tab = TabContextMenu.contextTab;
      gEnsoEmojiPicker
        .open(tab.iconImage, { emojiAsSVG: true })
        .then((icon) => {
          if (icon) {
            tab.ensoStaticIcon = icon;
          } else {
            delete tab.ensoStaticIcon;
          }
  ...
  updatePinnedTabContextMenu(contextTab) {
    ...
    const isEssential = contextTab.getAttribute("enso-essential");
    const ensoAddEssential = document.getElementById("context_enso-add-essential");
    const ensoResetPinnedTab = document.getElementById("context_enso-reset-pinned-tab");
    const ensoReplacePinnedUrl = document.getElementById(
      "context_enso-replace-pinned-url-with-current"
    );
    [ensoResetPinnedTab, ensoReplacePinnedUrl].forEach((element) => {
      if (element) {
        element.hidden = !isVisible;
        document.l10n.setArgs(element, { isEssential });
      }
    });
    ensoAddEssential.hidden = isEssential || !!contextTab.group;
    document.l10n
      .formatValue("tab-context-enso-add-essential-badge", {
        num: gBrowser._numZenEssentials,
        max: this.maxEssentialTabs,
      })
      .then((badgeText) => {
        ensoAddEssential.setAttribute("badge", badgeText);
      });
    document
      .getElementById("cmd_contextZenAddToEssentials")
      .setAttribute("disabled", !this.canEssentialBeAdded(contextTab));
    document.getElementById("context_closeTab").hidden = contextTab.hasAttribute("enso-essential");
    document.getElementById("context_enso-remove-essential").hidden = !isEssential;
    ...
    document.getElementById("context_enso-pinned-tab-separator").hidden = !isVisible;
    document.getElementById("context_enso-edit-tab-title").hidden =
      isEssential ||
  ...
  moveToAnotherTabContainerIfNecessary(event, draggedTab, movingTabs, dropIndex) {
      ...
      .map((tab) => {
        ...
        if (tab.ownerGlobal !== window) {
          ...
          if (
            ...
            tab.getAttribute("enso-workspace-id") != gEnsoWorkspaces.activeWorkspace
          ) {
            workspaceId = gEnsoWorkspaces.activeWorkspace;
            tab.ownerGlobal.gBrowser.selectedTab = tab.ownerGlobal.gBrowser._findTabToBlurTo(
              tab,
              movingTabs
            );
            tab.ownerGlobal.gEnsoWorkspaces.moveTabToWorkspace(tab, workspaceId);
          }
    ...
    try {
      const pinnedTabsTarget = event.target.closest(
        ":is(.enso-current-workspace-indicator, .enso-workspace-pinned-tabs-section)"
      );
      const essentialTabsTarget = event.target.closest(".enso-essentials-container");
      const tabsTarget = !pinnedTabsTarget;
      let currentEssenialContainer = gEnsoWorkspaces.getCurrentEssentialsContainer();
      ...
      movingTabs = movingTabs.filter((tab) =>
        gBrowser.isTabGroupLabel(tab) && tab.group?.isEnsoFolder
      ...
      for (const tab of movingTabs) {
        ...
        if (essentialTabsTarget) {
          if (!tab.hasAttribute("enso-essential") && !tab?.group?.hasAttribute("split-view-group")) {
            moved = true;
            isVertical = false;
        ...
        // Check for pinned tabs container
          ...
          if (!tab.pinned) {
            gBrowser.pinTab(tab);
          } else if (tab.hasAttribute("enso-essential")) {
            this.removeEssentials(tab, false);
            moved = true;
          }
        }
        // Check for normal tabs container
        else if (tabsTarget || event.target.id === "enso-tabs-wrapper") {
          if (tab.pinned && !tab.hasAttribute("enso-essential")) {
            gBrowser.unpinTab(tab);
            isRegularTabs = true;
          } else if (tab.hasAttribute("enso-essential")) {
        ...
        if (hasActuallyMoved) {
          ...
          const targetFolder = event.target.closest("enso-folder");
  ...
  onLocationChange(browser) {
    ...
    if (
      ...
      !tab.pinned ||
      tab.hasAttribute("enso-essential") ||
      !tab._ensoPinnedInitialState?.entry
    ) {
      return;
    }
    // Remove # and ? from the URL
    const pinUrl = tab._ensoPinnedInitialState.entry.url.split("#")[0];
  ...
  resetPinChangedUrl(tab) {
    if (!tab.hasAttribute("enso-pinned-changed")) {
      return;
    }
    tab.removeAttribute("enso-pinned-changed");
    tab.removeAttribute("had-enso-pinned-changed");
    tab.style.removeProperty("--enso-original-tab-icon");
  }
  ...
  pinHasChangedUrl(tab) {
    ...
    if (tab.group?.hasAttribute("split-view-group")) {
      tab.setAttribute("had-enso-pinned-changed", "true");
    } else {
      tab.setAttribute("enso-pinned-changed", "true");
    }
    tab.style.setProperty("--enso-original-tab-icon", `url(${tab._ensoPinnedInitialState.image})`);
  }
  ...
  removeTabContainersDragoverClass(hideIndicator = true) {
    ...
    if (hideIndicator) {
      gEnsoWorkspaces.activeWorkspaceIndicator?.removeAttribute("open");
    }
  }
  ...
  get dragIndicator() {
    if (!this._dragIndicator) {
      ...
      this._dragIndicator.id = "enso-drag-indicator";
  ...
  get expandedSidebarMode() {
    return document.documentElement.getAttribute("enso-sidebar-expanded") === "true";
  }
    ...
    return (
      !(
        (tab.getAttribute("usercontextid") || 0) !=
          gEnsoWorkspaces.getActiveWorkspaceFromCache().containerTabId &&
        gEnsoWorkspaces.containerSpecificEssentials
  ...
  applyDragoverClass(event, draggedTab) {
    ...
    const pinnedTabsTarget = event.target.closest(".enso-workspace-pinned-tabs-section");
    const essentialTabsTarget = event.target.closest(".enso-essentials-container");
    const tabsTarget = event.target.closest(".enso-workspace-normal-tabs-section");
    const folderTarget = event.target.closest("enso-folder");
    ...
    draggedTab = draggedTab?.group?.hasAttribute("split-view-group")
      ? draggedTab.group
      : draggedTab;
    const isHoveringIndicator = !!event.target.closest(".enso-current-workspace-indicator");
    if (isHoveringIndicator) {
      this.removeTabContainersDragoverClass(false);
      gEnsoWorkspaces.activeWorkspaceIndicator?.setAttribute("open", true);
    } else {
      gEnsoWorkspaces.activeWorkspaceIndicator?.removeAttribute("open");
    }

    if (draggedTab?._dragData?.movingTabs) {
      gEnsoFolders.ungroupTabsFromActiveGroups(draggedTab._dragData.movingTabs);
    }
    ...
    if (essentialTabsTarget) {
      if (!draggedTab.hasAttribute("enso-essential") && this.canEssentialBeAdded(draggedTab)) {
        shouldAddDragOverElement = true;
        isVertical = false;
      }
    } else if (pinnedTabsTarget) {
      if (draggedTab.hasAttribute("enso-essential")) {
        shouldAddDragOverElement = true;
      }
    } else if (tabsTarget) {
      if (draggedTab.hasAttribute("enso-essential")) {
...
window.gEnsoPinnedTabManager = new nsEnsoPinnedTabManager();
