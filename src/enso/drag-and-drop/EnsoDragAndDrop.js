...
// Wrap in a block to prevent leaking to window scope.
{
  ...
  const isEssentialsPromo = (element) => element?.tagName.toUpperCase() == "ENSO-ESSENTIALS-PROMO";
  ...
  const elementToMove = (element) => {
    if (
      !element ||
      element.closest(".enso-current-workspace-indicator") ||
      element.hasAttribute("split-view-group") ||
      element.classList.contains("enso-drop-target") ||
  ...
  window.EnsoDragAndDrop = class extends window.TabDragAndDrop {
    ...
    constructor(tabbrowserTabs) {
      ...
      XPCOMUtils.defineLazyPreferenceGetter(
        ...
        "enso.tabs.dnd-switch-space-delay",
        1000
      );
      ...
      ChromeUtils.defineESModuleGetters(
        this,
        {
          createEnsoEssentialsPromo:
            "chrome://browser/content/enso-components/EnsoEssentialsPromo.mjs",
        },
        { global: "current" }
      );
    }
    ...
    #createDragImageForTabs(movingTabs) {
      const periphery = gEnsoWorkspaces.activeWorkspaceElement.querySelector(
    ...
    _animateTabMove(event) {
      ...
      if (event.target.closest("#enso-essentials") && !isEssentialsPromo(event.target)) {
        if (!isTab(draggedTab)) {
      ...
      let periphery = gEnsoWorkspaces.activeWorkspaceElement.querySelector(
        "#tabbrowser-arrowscrollbox-periphery"
      );
    ...
    get #dragShiftableItems() {
      const separator = gEnsoWorkspaces.pinnedTabsContainer.querySelector(
        ".pinned-tabs-container-separator"
      );
      // Make sure to always return the separator at the start of the array
      return Services.prefs.getBoolPref("enso.view.show-newtab-button-top")
        ? [separator, gEnsoWorkspaces.activeWorkspaceElement.newTabButton]
        : [separator];
    }
    ...
    #shouldSwitchSpace(event) {
      const padding = Services.prefs.getIntPref("enso.workspaces.dnd-switch-padding");
      ...
      if (!(gEnsoCompactModeManager.preference && gEnsoCompactModeManager.canHideSidebar)) {
            ...
            gEnsoWorkspaces
              ...
              .then((spaceChanged) => {
                ...
                const { isDarkMode, isExplicitMode } =
                  gEnsoThemePicker.getGradientForWorkspace(spaceChanged);
                for (let tab of tabs) {
                  if (isExplicitMode) {
    ...
    handle_windowDragLeave(event) {
      ...
      let allowedMargin = Services.prefs.getIntPref("enso.tabs.dnd-outside-window-margin", 5);
      const isOutOfWindow =
      ...
      if (isOutOfWindow && !this.#isOutOfWindow) {
        ...
        gEnsoViewSplitter.onBrowserDragEndToSplit(event, true);
    ...
    handle_drop(event) {
      this.clearSpaceSwitchTimer();
      gEnsoFolders.highlightGroupOnDragOver(null);
      super.handle_drop(event);
      this.#maybeClearVerticalPinnedGridDragOver();
      const dt = event.dataTransfer;
      const activeWorkspace = gEnsoWorkspaces.activeWorkspace;
      let draggedTab = dt.mozGetDataAt(TAB_DROP_TYPE, 0);
      if (draggedTab.ownerGlobal === window) {
        if (
          !draggedTab.hasAttribute("enso-essential") &&
          draggedTab.getAttribute("enso-workspace-id") != activeWorkspace
        ) {
          if (isTab(draggedTab)) {
            const movingTabs = draggedTab._dragData?.movingTabs || [draggedTab];
            for (let tab of movingTabs) {
              tab.setAttribute("enso-workspace-id", activeWorkspace);
            }
            gBrowser.selectedTab = draggedTab;
          } else if (isTabGroupLabel(draggedTab)) {
            draggedTab = draggedTab.group;
            gEnsoFolders.changeFolderToSpace(draggedTab, activeWorkspace, { hasDndSwitch: true });
          }
        }
      }
    ...
    handle_drop_transition(dropElement, draggedTab, movingTabs, dropBefore) {
      ...
      try {
        if (
          ...
          gReduceMotion ||
          !dropElement ||
          dropElement.hasAttribute("enso-essential") ||
          draggedTab.hasAttribute("enso-essential") ||
          draggedTab.getAttribute("enso-workspace-id") != gEnsoWorkspaces.activeWorkspace ||
          !dropElement.visible ||
          !draggedTab.visible ||
    ...
    handle_dragend(event) {
      ...
      let currentEssenialContainer = ownerGlobal.gEnsoWorkspaces.getCurrentEssentialsContainer();
      ...
      delete ownerGlobal.gEnsoCompactModeManager._isTabBeingDragged;
      if (dt.dropEffect !== "move") {
        ownerGlobal.gEnsoCompactModeManager._clearAllHoverStates();
      }
    }
    ...
    #applyDragOverBackground(element) {
      ...
      this.#dragOverBackground.id = "enso-dragover-background";
    ...
    clearDragOverVisuals() {
      this.#removeDragOverBackground();
      gEnsoPinnedTabManager.removeTabContainersDragoverClass();
    }

    // eslint-disable-next-line complexity
    #applyDragoverIndicator(event, dropElement, movingTabs, draggedTab) {
      const separation = 4;
      const dropZoneSelector = ":is(.enso-drop-target)";
      ...
      if (!dropElementFromEvent) {
        ...
        if (event.target.classList.contains("enso-workspace-empty-space") || hoveringPeriphery) {
          let lastTab = gBrowser.tabs.at(-1);
          dropElement =
            (hoveringPeriphery && Services.prefs.getBoolPref("enso.view.show-newtab-button-top")
          ...
          showIndicatorUnderNewTabButton = lastTab.hasAttribute("enso-empty-tab");
        }
      }
      ...
      if (!dropElement || dropElement.hasAttribute("enso-essential")) {
        this.clearDragOverVisuals();
        return null;
      }
      if (dropElement.hasAttribute("enso-empty-tab") && dropElement.group) {
      ...
      let isEnsoFolder = possibleFolderElement?.isEnsoFolder;
      let canHightlightGroup =
        gEnsoFolders.highlightGroupOnDragOver(possibleFolderElement, movingTabs) || !isEnsoFolder;
      ...
      let threshold = Services.prefs.getIntPref("enso.tabs.folder-dragover-threshold-percent") / 100;
      let dropIntoFolder =
        isEnsoFolder &&
      ...
      if (
        isTabGroupLabel(draggedTab) &&
        draggedTab.group?.isEnsoFolder &&
        (((isTab(dropElement) || dropElement.hasAttribute("split-view-group")) &&
          (!dropElement.pinned || dropElement.hasAttribute("enso-essential"))) ||
      ...
      if (
        ...
        const indicator = gEnsoPinnedTabManager.dragIndicator;
        let top = 0;
        threshold =
          Services.prefs.getIntPref("browser.tabs.dragDrop.moveOverThresholdPercent") / 100;
        ...
        this.#removeDragOverBackground();
        if (!isTab(dropElement) && dropElement?.parentElement?.isEnsoFolder) {
          dropElement = dropElement.parentElement;
        }
      } else if (dropElement.classList.contains("enso-drop-target") && canHightlightGroup) {
        shouldPlayHapticFeedback =
          this.#applyDragOverBackground(dropElement) && !gEnsoPinnedTabManager._dragIndicator;
        gEnsoPinnedTabManager.removeTabContainersDragoverClass();
        dropElement = dropElement.parentElement?.labelElement || dropElement;
        if (dropElement.classList.contains("enso-current-workspace-indicator")) {
          dropElement =
    ...
    #animateVerticalPinnedGridDragOver(event) {
      ...
      let movingTabs = dragData.movingTabs;
      if (
        !gEnsoPinnedTabManager.canEssentialBeAdded(draggedTab) &&
        !draggedTab.hasAttribute("enso-essential")
      ) {
        return;
      }
      let essentialsPromoStatus = this.createEnsoEssentialsPromo();
      this.clearDragOverVisuals();
      ...
      if (!this._fakeEssentialTab) {
        ...
        if (!draggedTab.hasAttribute("enso-essential")) {
          event.target.closest(".enso-essentials-container").appendChild(this._fakeEssentialTab);
          gEnsoWorkspaces.updateTabsContainers();
    ...
    #makeDragImageEssential(event) {
      ...
      gEnsoPinnedTabManager.setEssentialTabIcon(tab);
