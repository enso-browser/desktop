...
import { nsZenMenuBar } from "chrome://browser/content/enso-components/EnsoMenubar.mjs";

window.gEnsoUIManager = {
  _popupTrackingElements: [],
  _hoverPausedForExpand: false,
  _hasLoadedDOM: false,
  testingEnabled: Services.prefs.getBoolPref("enso.testing.enabled", false),
  profilingEnabled: Services.prefs.getBoolPref("enso.testing.profiling.enabled", false),
  ...
  init() {
    window.gEnsoMenubar = new nsZenMenuBar();
    ...
    ChromeUtils.defineLazyGetter(this, "motion", () => {
      Services.scriptloader.loadSubScript(
        "chrome://browser/content/enso-vendor/motion.min.mjs",
        window
      );
    ...
    ChromeUtils.defineLazyGetter(this, "_toastContainer", () => {
      return document.getElementById("enso-toast-container");
    });

    new ResizeObserver(
      gEnsoCommonActions.throttle(
        gEnsoCompactModeManager.getAndApplySidebarWidth.bind(gEnsoCompactModeManager),
        Services.prefs.getIntPref("enso.view.sidebar-height-throttle", 500)
      )
    ).observe(gNavToolbox);

    gEnsoWorkspaces.promiseInitialized.finally(() => {
    ...
    window.addEventListener(
      "Enso:UrlbarSearchModeChanged",
      this.onUrlbarSearchModeChanged.bind(this)
    );

    gEnsoMediaController.init();
    gEnsoVerticalTabsManager.init();
   ...
   * This is not using gEnsoUIManager.motion, because motion library has some issues
  ...
  _addNewCustomizableButtonsIfNeeded() {
    const kPref = "enso.ui.migration.compact-mode-button-added";
    let navbarPlacements = CustomizableUI.getWidgetIdsInArea("enso-sidebar-top-buttons");
    try {
      if (!navbarPlacements.length && !Services.prefs.getBoolPref(kPref, false)) {
        CustomizableUI.addWidgetToArea("enso-toggle-compact-mode", "enso-sidebar-top-buttons", 0);
        gEnsoVerticalTabsManager._topButtonsSeparatorElement.before(
          document.getElementById("enso-toggle-compact-mode")
        );
      }
    } catch (e) {
  ...
  _initBookmarkCollapseListener() {
    ...
    if (!bookmarkToolbar.hasAttribute("collapsed")) {
      ...
      document.documentElement.setAttribute("enso-has-bookmarks", "true");
    }
    bookmarkToolbar.addEventListener("toolbarvisibilitychange", (event) => {
      ...
      if (visible) {
        ...
        document.documentElement.removeAttribute("enso-has-bookmarks");
      }
    });
  },

  _initOmnibox() {
    const { registerEnsoUrlbarProviders } = ChromeUtils.importESModule(
      "resource:///modules/EnsoUBProvider.sys.mjs"
    );
    const { nsEnsoSiteDataPanel: EnsoSiteDataPanel } = ChromeUtils.importESModule(
      "resource:///modules/EnsoSiteDataPanel.sys.mjs"
    );
    registerEnsoUrlbarProviders();
    window.gEnsoSiteDataPanel = new EnsoSiteDataPanel(window);
    gURLBar._ensoTrimURL = this.urlbarTrim.bind(this);
  },

  _debloatContextMenus() {
    if (!Services.prefs.getBoolPref("enso.view.context-menu.refresh", false)) {
      return;
    }
    const contextMenusToClean = [
      // Remove the 'new tab below' context menu.
      // reason: It doesn't properly work with enso and it's philosophy of not having
  ...
  _initCreateNewPopup() {
    const popup = document.getElementById("ensoCreateNewPopup");

    popup.addEventListener("popupshowing", () => {
      const button = document.getElementById("enso-create-new-button");
      if (!button) {
        return;
      }
  ...
  updateTabsToolbar() {
    ...
    gURLBar.style.setProperty("--enso-urlbar-width", `${Math.min(window.innerWidth / 2, 700)}px`);
    gEnsoVerticalTabsManager.actualWindowButtons.removeAttribute("enso-has-hover");
    gEnsoVerticalTabsManager.recalculateURLBarHeight();
  ...
  get tabsWrapper() {
    ...
    this._tabsWrapper = document.getElementById("enso-tabs-wrapper");
    return this._tabsWrapper;
  },
  ...
  onPopupShowing(showEvent) {
    for (const el of this._popupTrackingElements) {
      ...
      if (
        ...
        // Also see #10612 for the exclusion of the enso-appcontent-navbar-wrapper
        (showEvent.explicitOriginalTarget.tagName === "menupopup" &&
          el.id !== "enso-appcontent-navbar-wrapper")
      ) {
        continue;
      }
      document.removeEventListener("mousemove", this.__removeHasPopupAttribute);
      gEnsoCompactModeManager._setElementExpandAttribute(el, true, "has-popup-menu");
  ...
  onPopupHidden(hideEvent) {
    ...
    if (document.getElementById("main-window").matches(":hover")) {
      gEnsoCompactModeManager._setElementExpandAttribute(element, false, "has-popup-menu");
    } else {
  ...
  enableCommandsMode(event) {
    ...
    if (!currentSearchMode) {
      searchMode = {
        source: UrlbarUtils.RESULT_SOURCE.ENSO_ACTIONS,
        isPreview: true,
      };
    }
    gURLBar.removeAttribute("animate-searchmode");
  ...
  handleNewTab(werePassedURL, searchClipboard, where, overridePreferance = false) {
    ...
    gURLBar._ensoHandleUrlbarClose = this.handleUrlbarClose.bind(this);
    gURLBar.setAttribute("enso-newtab", true);
  ...
  handleUrlbarClose(onSwitch = false, onElementPicked = false) {
    ...
    // Reset URL bar state
    if (gURLBar._ensoHandleUrlbarClose) {
      gURLBar._ensoHandleUrlbarClose = null;
    }

    const isFocusedBefore = gURLBar.focused;
    setTimeout(() => {
      // We use this attribute on Tabbrowser::addTab
      gURLBar.removeAttribute("enso-newtab");
  ...
  urlbarTrim(aURL) {
    if (
      gEnsoVerticalTabsManager._hasSetSingleToolbar &&
  ...
  _createToastElement(messageId, options) {
    ...
    wrapper.classList.add("enso-toast");
  ...
  panelUIPosition(panel, anchor) {
    ...
    let inline = "topleft";
    if (anchor?.closest("#enso-sidebar-top-buttons")) {
      block = "topleft";
    }
    if (
      (gEnsoVerticalTabsManager._hasSetSingleToolbar && gEnsoVerticalTabsManager._prefsRightSide) ||
      (panel?.id === "enso-unified-site-data-panel" &&
        !gEnsoVerticalTabsManager._hasSetSingleToolbar) ||
      (panel?.id === "unified-extensions-panel" && gEnsoVerticalTabsManager._hasSetSingleToolbar)
    ) {
      block = "bottomright";
      inline = "topright";
    }
  ...
  getOpenUILinkWhere(url, browser, openUILinkWhere) {
    try {
      ...
      if (
        ...
        tab.pinned &&
        Services.prefs.getBoolPref("enso.tabs.open-pinned-in-new-tab")
      ) {
        return "tab";
      }
    } catch (e) {
...
window.gEnsoVerticalTabsManager = {
  init() {
    ...
    ChromeUtils.defineLazyGetter(this, "isWindowsStyledButtons", () => {
      return !(
        ...
        Services.prefs.getBoolPref("enso.view.experimental-force-window-controls-left")
      );
    });
    ...
    gEnsoCompactModeManager.addEventListener(updateEvent);
    ...
    if (!this.isWindowsStyledButtons) {
      document.documentElement.setAttribute("enso-window-buttons-reversed", true);
    }
  ...
  toggleExpand() {
    const newVal = !Services.prefs.getBoolPref("enso.view.sidebar-expanded");
    Services.prefs.setBoolPref("enso.view.sidebar-expanded", newVal);
    Services.prefs.setBoolPref("enso.view.use-single-toolbar", false);
  },
  ...
  initRightSideOrderContextMenu() {
    const kConfigKey = "enso.tabs.vertical.right-side";
    const fragment = window.MozXULElement.parseXULToFragment(`
      <menuitem id="enso-toolbar-context-tabs-right"
                type="checkbox"
                ${Services.prefs.getBoolPref(kConfigKey) ? 'checked="true"' : ""}
                data-lazy-l10n-id="enso-toolbar-context-tabs-right"
  ...
  get _topButtonsSeparatorElement() {
    ...
    this.__topButtonsSeparatorElement = document.getElementById(
      "enso-sidebar-top-buttons-separator"
    );
    return this.__topButtonsSeparatorElement;
  },
  ...
  animateItemOpen(aItem) {
    if (
      ...
      !aItem ||
      !gEnsoUIManager._hasLoadedDOM ||
      !aItem.isConnected ||
      // We do want to do some animations during testing with profiling enabled
      // so we can capture and improve them.
      (gEnsoUIManager.testingEnabled && !gEnsoUIManager.profilingEnabled) ||
      !gEnsoStartup.isReady ||
  ...
  animateTabClose(aTab) {
    if (aTab.hasAttribute("enso-essential") || aTab.group?.hasAttribute("split-view-group")) {
  ...
  async _preCustomize() {
    await this._multiWindowFeature.foreachWindowAsActive(async (browser) => {
      browser.gEnsoVerticalTabsManager._updateEvent({
        forCustomizableMode: true,
        dontRebuildAreas: true,
      });
    });
    this.rebuildAreas();
    this.navigatorToolbox.setAttribute("enso-sidebar-expanded", "true");
    document.documentElement.setAttribute("enso-sidebar-expanded", "true"); // force expanded sidebar
  },
  ...
  _postCustomize() {
    ...
    this._multiWindowFeature.foreachWindowAsActive(async (browser) => {
      browser.gEnsoVerticalTabsManager._updateEvent({ dontRebuildAreas: true });
    });
  },
  ...
  async _onPrefChange() {
    ...
    await this._multiWindowFeature.foreachWindowAsActive(async (browser) => {
      if (browser.gEnsoVerticalTabsManager._multiWindowFeature.windowIsActive(browser)) {
        return;
      }
      await browser.gEnsoVerticalTabsManager._waitPromise;
      browser.gEnsoVerticalTabsManager._updateEvent({ dontRebuildAreas: true });
      browser.gEnsoVerticalTabsManager._initWaitPromise();
    });
  ...
  _updateEvent({ forCustomizableMode = false, dontRebuildAreas = false } = {}) {
    ...
    try {
      ...
      const topButtons = document.getElementById("enso-sidebar-top-buttons");
      const isCompactMode = gEnsoCompactModeManager.preference && !forCustomizableMode;
      ...
      const buttonsTarget = document.getElementById("enso-sidebar-top-buttons-customization-target");
      if (isRightSide) {
        this.navigatorToolbox.setAttribute("enso-right-side", "true");
        document.documentElement.setAttribute("enso-right-side", "true");
      } else {
        this.navigatorToolbox.removeAttribute("enso-right-side");
        document.documentElement.removeAttribute("enso-right-side");
      }

      delete this._hadSidebarCollapse;
      if (isSidebarExpanded) {
        this._hadSidebarCollapse = !document.documentElement.hasAttribute("enso-sidebar-expanded");
        this.navigatorToolbox.setAttribute("enso-sidebar-expanded", "true");
        document.documentElement.setAttribute("enso-sidebar-expanded", "true");
        gBrowser.tabContainer.setAttribute("expanded", "true");
      } else {
        this.navigatorToolbox.removeAttribute("enso-sidebar-expanded");
        document.documentElement.removeAttribute("enso-sidebar-expanded");
        gBrowser.tabContainer.removeAttribute("expanded");
      }

      const appContentNavbarContaienr = document.getElementById("enso-appcontent-navbar-container");
      const appContentNavbarWrapper = document.getElementById("enso-appcontent-navbar-wrapper");
      ...
      if (!this.navigatorToolbox.hasAttribute("enso-right-side") && !isCompactMode) {
      ...
      if (isSingleToolbar) {
        ...
        document.documentElement.setAttribute("enso-single-toolbar", true);
        ...
        const elements = document.querySelectorAll(
          '#enso-sidebar-top-buttons-customization-target > :is([cui-areatype="toolbar"], .chromeclass-toolbar-additional)'
        );
        for (const button of elements) {
          document.getElementById("nav-bar-customization-target").append(button);
        }
        this._topButtonsSeparatorElement.remove();
        document.documentElement.removeAttribute("enso-single-toolbar");
        const panelUIButton = document.getElementById("PanelUI-button");
      ...
      const canHideTabBarPref = Services.prefs.getBoolPref("enso.view.compact.hide-tabbar");
      ...
      if (
        ...
        Services.prefs.getBoolPref("enso.view.overflow-webext-toolbar", true)
      ) {
        topButtons.setAttribute("addon-webext-overflowtarget", "enso-overflow-extensions-list");
      } else {
        topButtons.setAttribute("addon-webext-overflowtarget", "overflowed-extensions-list");
      }

      gEnsoCompactModeManager.updateCompactModeContext(isSingleToolbar);

      // Always move the splitter next to the sidebar
      const splitter = document.getElementById("enso-sidebar-splitter");
  ...
  rebuildAreas() {
    CustomizableUI.ensoInternalCU._rebuildRegisteredAreas(/* ensoDontRebuildCollapsed */ true);
  },

  _updateMaxWidth() {
    const maxWidth = Services.prefs.getIntPref("enso.view.sidebar-expanded.max-width");
  ...
  get expandButton() {
    ...
    this._expandButton = document.getElementById("enso-expand-sidebar-button");
    return this._expandButton;
  },

  toggleTabsOnRight() {
    const newVal = !Services.prefs.getBoolPref("enso.tabs.vertical.right-side");
    Services.prefs.setBoolPref("enso.tabs.vertical.right-side", newVal);
  },

  appendCustomizableItem(target, child, placements) {
    if (
      target.id === "enso-sidebar-top-buttons-customization-target" &&
      this._hasSetSingleToolbar &&
  ...
  async renameTabKeydown(event) {
    ...
    if (event.key === "Enter") {
      ...
      document.documentElement.removeAttribute("enso-renaming-tab");
      input.remove();
      if (!isTab) {
        ...
        if (hasChanged || (this._tabEdited.ensoStaticLabel && newName)) {
          this._tabEdited.ensoStaticLabel = newName;
          gBrowser._setTabLabel(this._tabEdited, newName);
          gEnsoUIManager.showToast("enso-tabs-renamed");
        } else {
          delete this._tabEdited.ensoStaticLabel;
  ...
  renameTabStart(event) {
    ...
    if (event.target.id === "context_enso-edit-tab-title") {
      target = TabContextMenu.contextTab;
    }
    const isTab = !!target.closest(".tabbrowser-tab");
    if (
      this._tabEdited ||
      ((!Services.prefs.getBoolPref("enso.tabs.rename-tabs") ||
        (Services.prefs.getBoolPref("browser.tabs.closeTabByDblclick") &&
          event.type === "dblclick")) &&
        isTab) ||
      !gEnsoVerticalTabsManager._prefsSidebarExpanded
    ) {
      return;
    }
    ...
    this._tabEdited =
      ...
      target.closest(".enso-current-workspace-indicator-name") ||
      (event.explicit && target.closest(".tab-group-label"));
    if (!this._tabEdited || (this._tabEdited.hasAttribute("enso-essential") && isTab)) {
      this._tabEdited = null;
      return;
    }
    gEnsoFolders.cancelPopupTimer();
    event.stopPropagation?.();
    document.documentElement.setAttribute("enso-renaming-tab", "true");
