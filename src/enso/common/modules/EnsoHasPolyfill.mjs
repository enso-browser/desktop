    ...
    const updateState = () => {
      ...
      if (exists === shouldExist) {
        if (!element.hasAttribute(stateAttribute)) {
          gEnsoCompactModeManager._setElementExpandAttribute(element, true, stateAttribute);
        }
      } else if (element.hasAttribute(stateAttribute)) {
        gEnsoCompactModeManager._setElementExpandAttribute(element, false, stateAttribute);
      }
    };
...
window.EnsoHasPolyfill = hasPolyfillInstance;
