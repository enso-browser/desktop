...
export class nsEnsoDOMOperatedFeature {
  constructor() {
...
window.gEnsoCommonActions = {
  copyCurrentURLToClipboard() {
    const [currentUrl, ClipboardHelper] = gURLBar.ensoStrippedURI;
    const displaySpec = currentUrl.displaySpec;
    ClipboardHelper.copyString(displaySpec);
    let button;
    /* eslint-disable mozilla/valid-services */
    if (Services.enso.canShare() && displaySpec.startsWith("http")) {
      button = {
        id: "enso-copy-current-url-button",
        command: (event) => {
    ...
    gEnsoUIManager.showToast("enso-copy-current-url-confirmation", { button, timeout: 3000 });
  },
  ...
  copyCurrentURLAsMarkdownToClipboard() {
    ...
    gEnsoUIManager.showToast("enso-copy-current-url-confirmation", { timeout: 3000 });
  },
   ...
   * Respects the user preference enso.tabs.close-on-back-with-no-history.
   *
   * @returns {boolean} True if the tab should be closed on back
   */
  shouldCloseTabOnBack() {
    if (!Services.prefs.getBoolPref("enso.tabs.close-on-back-with-no-history", true)) {
      return false;
    }
    const tab = gBrowser.selectedTab;
    return Boolean(tab.owner && !tab.pinned && !tab.hasAttribute("enso-empty-tab"));
  },
};
