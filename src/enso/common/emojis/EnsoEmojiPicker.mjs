...
class nsEnsoEmojiPicker extends nsEnsoDOMOperatedFeature {
  #panel;
  ...
  init() {
    this.#panel = document.getElementById("PanelUI-enso-emojis-picker");
  ...
  handleEvent(event) {
    switch (event.type) {
      ...
      case "popuphidden":
        ...
        break;
      case "command":
        if (event.target.id === "PanelUI-enso-emojis-picker-none") {
          this.#selectEmoji(null);
        } else if (event.target.id === "PanelUI-enso-emojis-picker-change-emojis") {
          this.#changePage(false);
        } else if (event.target.id === "PanelUI-enso-emojis-picker-change-svg") {
  ...
  get #emojis() {
    ...
    Services.scriptloader.loadSubScript(
      "chrome://browser/content/enso-components/EnsoEmojisData.min.mjs",
      lazy
    );
    /* eslint-disable mozilla/valid-lazy */
    this._emojis = lazy.EnsoEmojisData;
    return this._emojis;
  }

  get emojiList() {
    return document.getElementById("PanelUI-enso-emojis-picker-list");
  }

  get svgList() {
    return document.getElementById("PanelUI-enso-emojis-picker-svgs");
  }

  get searchInput() {
    return document.getElementById("PanelUI-enso-emojis-picker-search");
  }

  #changePage(toSvg = false) {
    const itemToScroll = toSvg
      ? this.svgList
      : document.getElementById("PanelUI-enso-emojis-picker-pages").querySelector('[emojis="true"]');
    ...
    const button = document.getElementById(
      `PanelUI-enso-emojis-picker-change-${toSvg ? "svg" : "emojis"}`
    );
    const otherButton = document.getElementById(
      `PanelUI-enso-emojis-picker-change-${toSvg ? "emojis" : "svg"}`
    );
  ...
  async #onPopupShowing(event) {
    ...
    if (allowEmojis) {
      ...
      for (const emoji of this.#emojis) {
        ...
        item.className = "toolbarbutton-1 enso-emojis-picker-emoji";
    ...
    for (const icon of SVG_ICONS) {
      const item = document.createXULElement("toolbarbutton");
      item.className = "toolbarbutton-1 enso-emojis-picker-svg";
  ...
  #onPopupHidden(event) {
    ...
    this.#anchor.removeAttribute("enso-emoji-open");
    this.#anchor.parentElement.removeAttribute("enso-emoji-open");
    this.#anchor = null;
  }
  ...
  open(anchor, { onlySvgIcons = false, emojiAsSVG = false } = {}) {
    ...
    this.#anchor = anchor;
    this.#anchor.setAttribute("enso-emoji-open", "true");
    this.#anchor.parentElement.setAttribute("enso-emoji-open", "true");
    if (onlySvgIcons) {
  ...
  getSVGURL(icon) {
    return `chrome://browser/skin/enso-icons/selectable/${icon}`;
  }
}

window.gEnsoEmojiPicker = new nsEnsoEmojiPicker();
