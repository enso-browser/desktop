 ...
 *   <script src="chrome://browser/content/ensoThemeModifier.js"></script>
 *
 ...
 */
{
  ...
  const kZenThemePrefsList = [
    ...
    "enso.theme.border-radius",
    "enso.theme.content-element-separation",
  ];
  const kZenMaxElementSeparation = 12;

  /**
   * EnsoThemeModifier controls the application of theme data to the browser,
  ...
  window.EnsoThemeModifier = {
    ...
    listenForEvents() {
      ...
      const eventsForSeparation = [
        "EnsoViewSplitter:SplitViewDeactivated",
        "EnsoViewSplitter:SplitViewActivated",
        "fullscreen",
        "EnsoCompactMode:Toggled",
      ];
    ...
    updateBorderRadius() {
      const borderRadius = Services.prefs.getIntPref("enso.theme.border-radius", -1);
      ...
      if (borderRadius == -1) {
        if (AppConstants.platform == "macosx") {
          ...
          document.documentElement.style.setProperty("--enso-border-radius", targetRadius + "px");
          ...
          document.documentElement.style.setProperty("--enso-border-radius", "8px");
        }
      } else {
        // Use the overridden value
        document.documentElement.style.setProperty("--enso-border-radius", borderRadius + "px");
      }
    },
    ...
    updateElementSeparation() {
      ...
      if (
        ...
        window.gEnsoCompactModeManager?.preference &&
        !document.getElementById("tabbrowser-tabbox")?.hasAttribute("enso-split-view") &&
        Services.prefs.getBoolPref("enso.view.borderless-fullscreen", true)
      ) {
        separation = 0;
      }
      ...
      document.documentElement.style.setProperty("--enso-element-separation", separation + "px");
      if (separation == kMinElementSeparation) {
        document.documentElement.setAttribute("enso-no-padding", true);
      } else {
        document.documentElement.removeAttribute("enso-no-padding");
      }
    },

    get elementSeparation() {
      return Math.min(
        Services.prefs.getIntPref("enso.theme.content-element-separation"),
     ...
     */
    updateAccentColor() {
      const accentColor = Services.prefs.getStringPref("enso.theme.accent-color");
      document.documentElement.style.setProperty("--enso-primary-color", accentColor);
    },
  };

  if (typeof Services !== "undefined") {
    EnsoThemeModifier.init();
  }
}
