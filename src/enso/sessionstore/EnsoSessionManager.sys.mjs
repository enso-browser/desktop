...
ChromeUtils.defineESModuleGetters(lazy, {
  ...
  gWindowSyncEnabled: "resource:///modules/enso/EnsoWindowSync.sys.mjs",
  gSyncOnlyPinnedTabs: "resource:///modules/enso/EnsoWindowSync.sys.mjs",
  DeferredTask: "resource://gre/modules/DeferredTask.sys.mjs",
});

XPCOMUtils.defineLazyPreferenceGetter(lazy, "gShouldLog", "enso.session-store.log", true);
XPCOMUtils.defineLazyPreferenceGetter(
  lazy,
  "gMaxSessionBackups",
  "enso.session-store.max-backups",
  20
);
XPCOMUtils.defineLazyPreferenceGetter(
  lazy,
  "gBackupHourSpan",
  "enso.session-store.backup-hour-span",
  3
);

const SHOULD_BACKUP_FILE = Services.prefs.getBoolPref("enso.session-store.backup-file", true);
const FILE_NAME = "enso-sessions.jsonlz4";
...
export class nsEnsoSessionManager {
  /**
   ...
   *
   * @type {nsEnsoSidebarObject}
   */
  #sidebarObject = new nsEnsoSidebarObject();
  /**
  ...
  log(...args) {
    if (lazy.gShouldLog) {
      ...
      console.log("EnsoSessionManager:", ...args);
    }
  }
  ...
  get #backupFolderPath() {
    let profileDir = Services.dirsvc.get("ProfD", Ci.nsIFile).path;
    return PathUtils.join(profileDir, "enso-sessions-backup");
  }
  ...
  async #getDataFromDBForMigration() {
    try {
      ...
      let data = {};
      let rows = await db.execute("SELECT * FROM enso_workspaces ORDER BY created_at ASC");
      ...
      rows = await db.execute("SELECT * FROM enso_pins ORDER BY position ASC");
  ...
  async readFile() {
    ...
    this.init();
    try {
      this.log("Reading Enso session file from disk");
      let promises = [];
      ...
      console.error("EnsoSessionManager: Failed to read session file", e);
    }
  ...
  onCrashCheckpoints(initialState) {
    ...
    const allowRestoreUnsynced = Services.prefs.getBoolPref(
      "enso.session-store.restore-unsynced-windows",
      true
    );
    if (!this._shouldRunMigration) {
      this.log(`Restoring Enso session data into ${initialState.windows?.length || 0} windows`);
      for (let i = 0; i < initialState.windows.length; i++) {
        let winData = initialState.windows[i];
    ...
    for (const winData of initialState?.windows || []) {
      ...
      if (winData.tabs) {
        for (const tabData of winData.tabs) {
          let storeId = tabData.ensoSyncId || tabData.ensoPinnedId;
          ...
          if (pinData) {
            tabData.ensoStaticLabel = pinData.editedTitle ? pinData.title : undefined;
          }
        }
      }
    }
  ...
  saveState(state) {
    ...
    this.log(`Saving Enso session data with ${sidebar.tabs?.length || 0} tabs`);
  }
  ...
  async #createBackupsIfNeeded() {
    ...
    try {
      ...
      const todayFileName = `enso-sessions-${dateToUse}.jsonlz4`;
      ...
      files = files.filter((file) => file.startsWith("enso-sessions-")).sort();
      ...
      console.error("EnsoSessionManager: Failed to create session file backups", e);
    }
  }
  ...
  maybeSaveClosedWindow(aWinData, isLastWindow) {
    ...
    this.log("Saving closed window session data into Enso session store");
  ...
  #filterUnusedTabs(tabs) {
    return tabs.filter((tab) => {
      ...
      return !(tab.ensoIsEmpty && !tab.groupId);
    });
  }
  ...
  #collectTabsData(sidebarData, aStateWindows) {
    ...
    for (const window of aStateWindows) {
      // Only accept the tabs with `_ensoIsActiveTab` set to true from
      ...
      for (const tabData of window.tabs || []) {
        if (!tabIdRelationMap.has(tabData.ensoSyncId) || tabData._ensoIsActiveTab) {
          tabIdRelationMap.set(tabData.ensoSyncId, tabData);
        }
      }
    }
  ...
  /**
   * Restores a new window with Enso session data. This should be called
  ...
  restoreNewWindow(aWindow, SessionStoreInternal, fromClosedWindow = false) {
    if (aWindow.gEnsoWorkspaces?.privateWindowOrDisabled) {
      return;
    }
    this.log("Restoring new window with Enso session data");
  ...
  onNewEmptySession(aWindow) {
    this.log("Restoring empty session with Enso session data");
    aWindow.gEnsoWorkspaces.restoreWorkspacesFromSessionStore({
...
export const EnsoSessionStore = new nsEnsoSessionManager();
