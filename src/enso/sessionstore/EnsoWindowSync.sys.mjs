...
XPCOMUtils.defineLazyPreferenceGetter(lazy, "gWindowSyncEnabled", "enso.window-sync.enabled", true);
...
XPCOMUtils.defineLazyPreferenceGetter(lazy, "gShouldLog", "enso.window-sync.log", true);
...
class nsEnsoWindowSync {
  ...
  #browserWindows = {
    *[Symbol.iterator]() {
      for (let window of lazy.BrowserWindowTracker.orderedWindows) {
        if (window.__SSi && !window.closed && !window.gEnsoWorkspaces?.privateWindowOrDisabled) {
          yield window;
        }
      }
    },
  ...
  log(...args) {
    if (lazy.gShouldLog) {
      ...
      console.info("EnsoWindowSync:", ...args);
    }
  }
  ...
  #onWindowBeforeShow(aWindow) {
    if (
      aWindow.gEnsoWindowSync ||
      aWindow.document.documentElement.hasAttribute("enso-unsynced-window")
    ) {
      return;
    }
    ...
    // 1. We are passing `enso-unsynced` in the window arguments.
    ...
    let forcedSync = !aWindow.gEnsoWorkspaces?.privateWindowOrDisabled;
    ...
    if (lazy.PrivateBrowsingUtils.isWindowPrivate(aWindow)) {
      aWindow._ensoStartupSyncFlag = "synced";
    }
    if (aWindow._ensoStartupSyncFlag === "synced") {
      forcedSync = true;
    } else if (aWindow._ensoStartupSyncFlag === "unsynced") {
      hasUnsyncedArg = true;
    }
    delete aWindow._ensoStartupSyncFlag;
    if (
      !forcedSync &&
      (hasUnsyncedArg ||
      ...
      aWindow.document.documentElement.setAttribute("enso-unsynced-window", "true");
      for (let eventName of UNSYNCED_WINDOW_EVENTS) {
        aWindow.addEventListener(eventName, this, true);
      }
      return;
    }
    aWindow.gEnsoWindowSync = this;
  ...
  #runOnAllWindows(aWindow, aCallback) {
    for (let window of this.#browserWindows) {
      if (window !== aWindow && !window._ensoClosingWindow) {
  ...
  handleEvent(aEvent) {
    ...
    if (
      !window.gEnsoStartup.isReady ||
      !window.gEnsoWorkspaces?.shouldHaveWorkspaces ||
      window._ensoClosingWindow
    ) {
      return;
    }
  ...
  #syncItemWithOriginal(aOriginalItem, aTargetItem, aWindow, flags = 0) {
    ...
    const { gBrowser, gEnsoFolders } = aWindow;
    if (flags & SYNC_FLAG_ICON) {
      aTargetItem.ensoStaticIcon = aOriginalItem.ensoStaticIcon;
      if (gBrowser.isTab(aOriginalItem)) {
        gBrowser.setIcon(
          aTargetItem,
          aOriginalItem.getAttribute("image") || gBrowser.getIcon(aOriginalItem)
        );
      } else if (aOriginalItem.isEnsoFolder) {
        // Icons are a enso-only feature for tab groups.
        gEnsoFolders.setFolderUserIcon(aTargetItem, aOriginalItem.iconURL);
      }
    }
    if (flags & SYNC_FLAG_LABEL) {
      if (gBrowser.isTab(aOriginalItem)) {
        aTargetItem._ensoChangeLabelFlag = true;
        aTargetItem.ensoStaticLabel = aOriginalItem.ensoStaticLabel;
        gBrowser._setTabLabel(aTargetItem, aOriginalItem.label);
        delete aTargetItem._ensoChangeLabelFlag;
      } else if (gBrowser.isTabGroup(aOriginalItem)) {
        aTargetItem.label = aOriginalItem.label;
      }
    }
    if (flags & SYNC_FLAG_MOVE && !aTargetItem.hasAttribute("enso-empty-tab")) {
      this.#maybeSyncAttributeChange(aOriginalItem, aTargetItem, "enso-workspace-id");
  ...
  #syncItemPosition(aOriginalItem, aTargetItem, aWindow) {
    const { gBrowser, gEnsoPinnedTabManager } = aWindow;
    const originalIsEssential = aOriginalItem.hasAttribute("enso-essential");
    const targetIsEssential = aTargetItem.hasAttribute("enso-essential");
    ...
    if (aOriginalItem.hasAttribute("enso-glance-tab")) {
      return;
    }
    ...
    if (isTab) {
      if (originalIsEssential !== targetIsEssential) {
        if (originalIsEssential) {
          gEnsoPinnedTabManager.addToEssentials(aTargetItem);
        } else {
          gEnsoPinnedTabManager.removeEssentials(aTargetItem, /* unpin= */ !targetIsPinned);
        }
  ...
  #moveItemToMatchOriginal(aOriginalItem, aTargetItem, aWindow, { isEssential, isPinned }) {
    const { gBrowser, gEnsoWorkspaces } = aWindow;
    ...
    if (gBrowser.isTabGroup(originalSibling) || gBrowser.isTab(originalSibling)) {
      isFirstTab =
        !originalSibling.hasAttribute("id") || originalSibling.hasAttribute("enso-empty-tab");
    }

    gBrowser.ensoHandleTabMove(aTargetItem, () => {
      if (isFirstTab) {
        let container;
        ...
        if (isEssential) {
          container = gEnsoWorkspaces.getEssentialsSection(aTargetItem);
        } else {
          const workspaceId =
            aTargetItem.getAttribute("enso-workspace-id") ||
            aOriginalItem.ownerGlobal.gEnsoWorkspaces.activeWorkspace;
  ...
  #withRestoreTabProgressListener(aTab, callback, onClose = false) {
    ...
    if (!onClose && filter) {
      tabListener = new otherTabBrowser.ensoTabProgressListener(aTab, otherBrowser, true, false);
  ...
  #styleSwapedBrowsers(aOurTab, aOtherTab, callback = undefined, promiseToWait = null) {
    ...
    return new Promise(async (resolve) => {
      if (callback) {
        ...
        ourBrowser.removeAttribute("enso-pseudo-hidden");
        otherBrowser.setAttribute("enso-pseudo-hidden", "true");
        callback();
      } else {
        this.#maybeRemovePseudoImageForBrowser(ourBrowser);
        ourBrowser.removeAttribute("enso-pseudo-hidden");
  ...
  #createPseudoImageForBrowser(aBrowser, aSrc) {
    ...
    img.className = "enso-pseudo-browser-image";
    img.src = aSrc;
    aBrowser.after(img);
  }
  ...
  #maybeRemovePseudoImageForBrowser(aBrowser) {
    const elements = aBrowser.parentNode?.querySelectorAll(".enso-pseudo-browser-image");
    if (elements) {
   ...
   */
  #getActiveTabFromOtherWindows(aWindow, aTabId, filter = (tab) => tab?._ensoContentsVisible) {
  ...
  #moveAllActiveTabsToOtherWindowsForClose(aWindow) {
    ...
    if (!mostRecentWindow || !aWindow.gEnsoWorkspaces) {
      return;
    }
    const activeTabsOnClosedWindow = aWindow.gEnsoWorkspaces.allStoredTabs.filter(
      (tab) => tab._ensoContentsVisible
    );
    for (let tab of activeTabsOnClosedWindow) {
      ...
      if (targetTab) {
        ...
        targetTab._ensoContentsVisible = true;
        if (!tab.linkedBrowser) {
          continue;
        }
        delete tab._ensoContentsVisible;
  ...
  async #onTabSwitchOrWindowFocus(aWindow, aPreviousTab = null, ignoreSameTab = false) {
    ...
    if (aPreviousTab?._ensoContentsVisible && !activeTabs.includes(aPreviousTab)) {
      ...
      for (const tab of tabsToSwap) {
        ...
        if (otherTabToShow) {
          otherTabToShow._ensoContentsVisible = true;
          delete tab._ensoContentsVisible;
    ...
    for (const selectedTab of activeTabs) {
      if (selectedTab._ensoContentsVisible || selectedTab.hasAttribute("enso-empty-tab")) {
        continue;
      }
      const otherSelectedTab = this.#getActiveTabFromOtherWindows(aWindow, selectedTab.id);
      selectedTab._ensoContentsVisible = true;
      if (otherSelectedTab) {
        delete otherSelectedTab._ensoContentsVisible;
  ...
  setPinnedTabState(aTab) {
    return this.#maybeFlushTabState(aTab).finally(() => {
      ...
      this.#runOnAllWindows(null, (win) => {
        ...
        if (targetTab) {
          targetTab._ensoPinnedInitialState = initialState;
        }
      });
    });
  }
  ...
  propagateWorkspacesToAllWindows(aWorkspaces) {
    this.#runOnAllWindows(null, (win) => {
      win.gEnsoWorkspaces.propagateWorkspaces(aWorkspaces);
    });
  }
  ...
  moveTabsToSyncedWorkspace(aWindow, aWorkspaceId) {
    const tabsToMove = aWindow.gEnsoWorkspaces.allStoredTabs.filter(
      (tab) => !tab.hasAttribute("enso-empty-tab")
    );
    ...
    const moveAllTabsToWindow = async (allowSelected = false) => {
      ...
      await gEnsoWorkspaces.changeWorkspaceWithID(aWorkspaceId);
      gBrowser.selectedBrowser.focus();
    };
    if (!win) {
      this.log("No synced window found, creating a new one");
      win = aWindow.gBrowser.replaceTabWithWindow(selectedTab, {}, /* ensoForceSync = */ true);
      win.gEnsoWorkspaces.promiseInitialized.then(() => {
        moveAllTabsToWindow();
      });
      return;
    }
  ...
  #maybeEditAllTabsEntryImage(aTab) {
    if (!aTab?._ensoPinnedInitialState || aTab._ensoPinnedInitialState.image) {
      return;
    }
    let image = aTab.getAttribute("image") || aTab.ownerGlobal.gBrowser.getIcon(aTab);
    this.#runOnAllWindows(null, (win) => {
      const targetTab = this.getItemFromWindow(win, aTab.id);
      if (targetTab) {
        targetTab._ensoPinnedInitialState.image = image;
      }
    });
  }
  ...
  on_TabOpen(aEvent, { duringPinning = false } = {}) {
    ...
    const isUnsyncedWindow = window.gEnsoWorkspaces.privateWindowOrDisabled;
    ...
    this.#runOnAllWindows(window, (win) => {
      const newTab = win.gBrowser.addTrustedTab("about:blank", {
        ...
        _forEnsoEmptyTab: tab.hasAttribute("enso-empty-tab"),
      });
      newTab.id = tab.id;
    ...
    if (duringPinning && tab?.splitView) {
      this.on_EnsoSplitViewTabsSplit({ target: tab.group });
    }
    this.#maybeFlushTabState(tab);
  }

  on_EnsoTabIconChanged(aEvent) {
    if (!aEvent.target?._ensoContentsVisible) {
  ...
  on_TabPinned(aEvent) {
    ...
    let tabStatePromise;
    if (!tab._ensoPinnedInitialState) {
  ...
  on_TabUnpinned(aEvent) {
    ...
    this.#runOnAllWindows(null, (win) => {
      ...
      if (targetTab) {
        delete targetTab._ensoPinnedInitialState;
      }
    });
  ...
  on_SSWindowClosing(aEvent) {
    ...
    window._ensoClosingWindow = true;
    for (let eventName of EVENTS) {
      window.removeEventListener(eventName, this);
    }
    delete window.gEnsoWindowSync;
  ...
  on_TabGroupCreate(aEvent) {
    ...
    const isFolder = tabGroup.isEnsoFolder;
  ...
  on_EnsoTabRemovedFromSplit(aEvent) {
    const tab = aEvent.target;
    const window = tab.ownerGlobal;
    this.#runOnAllWindows(window, (win) => {
      const targetTab = this.getItemFromWindow(win, tab.id);
      if (targetTab && win.gEnsoViewSplitter) {
        win.gEnsoViewSplitter.removeTabFromGroup(targetTab);
      }
    });
  }

  on_EnsoSplitViewTabsSplit(aEvent) {
    const tabGroup = aEvent.target;
    const window = tabGroup.ownerGlobal;
    ...
    this.#runOnAllWindows(window, (win) => {
      const otherWindowTabs = tabs
        ...
        .filter(Boolean);
      if (otherWindowTabs.length && win.gEnsoViewSplitter) {
        const group = win.gEnsoViewSplitter.splitTabs(otherWindowTabs, undefined, -1, {
...
export const EnsoWindowSync = new nsEnsoWindowSync();
