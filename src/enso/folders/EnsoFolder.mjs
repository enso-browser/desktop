...
export class nsZenFolder extends MozTabbrowserTabGroup {
  ...
  static markup = `
      <hbox class="tab-group-label-container enso-drop-target" pack="center">
        <html:div class="tab-group-folder-icon"/>
        <label class="tab-group-label" role="button"/>
        <image class="tab-reset-button reset-icon" role="button" keyNav="false" data-l10n-id="enso-folders-unload-all-tooltip"/>
      </hbox>
      <html:div class="tab-group-container">
        <html:div class="enso-tab-group-start" />
      </html:div>
  ...
  static rawIcon = new DOMParser().parseFromString(
    `
      <svg width="28" height="28" viewBox="0 0 27 27" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <!--Back Folder (path)-->
        <path class="back" d="M8 5.625H11.9473C12.4866 5.625 13.0105 5.80861 13.4316 6.14551L14.2881 6.83105C14.9308 7.34508 15.7298 7.625 16.5527 7.625H20C21.3117 7.625 22.375 8.68832 22.375 10V20C22.375 21.3117 21.3117 22.375 20 22.375H8C6.68832 22.375 5.625 21.3117 5.625 20V8C5.625 6.68832 6.68832 5.625 8 5.625Z" style="fill: var(--enso-folder-behind-bgcolor);">
        </path>
        <path class="back" d="M8 5.625H11.9473C12.4866 5.625 13.0105 5.80861 13.4316 6.14551L14.2881 6.83105C14.9308 7.34508 15.7298 7.625 16.5527 7.625H20C21.3117 7.625 22.375 8.68832 22.375 10V20C22.375 21.3117 21.3117 22.375 20 22.375H8C6.68832 22.375 5.625 21.3117 5.625 20V8C5.625 6.68832 6.68832 5.625 8 5.625Z" style="stroke-width: 1.5px; stroke: var(--enso-folder-stroke); fill: url(#gradient-0); fill-opacity: 0.1;">
        </path>
        <!--Front Folder (rect)-->
        <rect class="front" x="5.625" y="9.625" width="16.75" height="12.75" rx="2.375" style="fill: var(--enso-folder-front-bgcolor);">
        </rect>
        <rect class="front" x="5.625" y="9.625" width="16.75" height="12.75" rx="2.375" style="stroke-width: 1.5px; stroke: var(--enso-folder-stroke); fill: url(#gradient-1); fill-opacity: 0.1;">
        </rect>
        <!--Icon (g)-->
        <g class="icon" style="fill: var(--enso-folder-stroke);">
          <image href="" height="10" width="10"/>
        </g>
        <!--End Icon (g)-->
        <g class="dots" style="fill: var(--enso-folder-stroke);">
  ...
  connectedCallback() {
    ...
    this.labelElement.parentElement.setAttribute("context", "ensoFolderActions");
  ...
  get childActiveGroups() {
    if (this.tagName === "enso-workspace-collapsible-pins") {
      return Array.from(this.parentElement.querySelectorAll("enso-folder[has-active]"));
    }
    return Array.from(this.querySelectorAll("enso-folder[has-active]"));
  }
  ...
  createSubfolder() {
    ...
    gEnsoFolders.createFolder([], {
      renameFolder: !gEnsoUIManager.testingEnabled,
      label: "Subfolder",
  ...
  get allItems() {
    return [...this.groupContainer.children].filter(
      (child) =>
        !(
          child.classList.contains("enso-tab-group-start") ||
  ...
  get pinned() {
    return this.isEnsoFolder;
  }

  /**
   * Intentionally ignore attempts to change the pinned state.
   * ZenFolder instances determine their "pinned" status based on their type (isEnsoFolder)
   ...
   * while preserving the invariant that EnsoFolders cannot have their pinned state changed externally.
   */
  set pinned(value) {}
  ...
  async #unloadAllActiveTabs(event, noClose = false) {
    await gEnsoPinnedTabManager.onCloseTabShortcut(event, this.tabs, {
      noClose,
      alwaysUnload: true,
      folderToUnload: this,
    });
...
customElements.define("enso-folder", nsZenFolder);
