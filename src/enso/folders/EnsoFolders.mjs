...
function groupIsCollapsiblePins(group) {
  return group?.tagName.toLowerCase() === "enso-workspace-collapsible-pins";
}

class nsEnsoFolders extends nsEnsoDOMOperatedFeature {
  #ENSO_MAX_SUBFOLDERS = Services.prefs.getIntPref("enso.folders.max-subfolders", 5);
  ...
  init() {
    this.#foldersEnabled = !gEnsoWorkspaces.privateWindowOrDisabled;
  ...
  #initContextMenu() {
    const contextMenuItems = window.MozXULElement.parseXULToFragment(
      `<menuitem id="enso-context-menu-new-folder" data-l10n-id="enso-toolbar-context-new-folder"/>`
    );
    ...
    const contextMenuItemsToolbar = window.MozXULElement.parseXULToFragment(
      `<menuitem id="enso-context-menu-new-folder-toolbar" data-l10n-id="enso-toolbar-context-new-folder"/>`
    );
    document.getElementById("toolbar-context-openANewTab").after(contextMenuItemsToolbar);

    const folderActionsMenu = document.getElementById("ensoFolderActions");
    folderActionsMenu.addEventListener("popupshowing", (event) => {
      ...
      if (gBrowser.isTabGroupLabel(target)) {
        ...
        target.parentElement?.isEnsoFolder &&
        target?.classList.contains("tab-group-label-container")
      ) {
        folder = target.parentElement;
      }

      // We only want to rename enso-folders as firefox groups don't work well with this
      if (!folder?.isEnsoFolder) {
        return;
      }
      this.#lastFolderContextMenu = folder;

      const newSubfolderItem = document.getElementById("context_ensoFolderNewSubfolder");
      newSubfolderItem.setAttribute(
        "disabled",
        folder.level >= this.#ENSO_MAX_SUBFOLDERS - 1 ? "true" : "false"
      );

      const changeFolderSpace = document
        .getElementById("context_ensoChangeFolderSpace")
        .querySelector("menupopup");
      changeFolderSpace.innerHTML = "";
      for (const workspace of [...gEnsoWorkspaces.getWorkspaces()].reverse()) {
        const item = gEnsoWorkspaces.generateMenuItemForWorkspace(workspace);
        item.addEventListener("command", (event) => {
          ...
          this.changeFolderToSpace(
            ...
            event.target.closest("menuitem").getAttribute("enso-workspace-id")
          );
        });
  ...
  #initTabsPopup() {
    this.#popup = document.getElementById("enso-folder-tabs-popup");

    const search = this.#popup.querySelector("#enso-folder-tabs-list-search");
    const tabsList = this.#popup.querySelector("#enso-folder-tabs-list");
  ...
  #initEventListeners() {
    ...
    document.getElementById("enso-context-menu-new-folder").addEventListener("command", onNewFolder);
    document
      .getElementById("enso-context-menu-new-folder-toolbar")
  ...
  on_TabGrouped(event) {
    ...
    if (group.hasAttribute("split-view-group") && group.hasAttribute("enso-pinned-changed")) {
      // enso-pinned-changed remove it and set it to had-enso-pinned-changed to keep
      // track of the original pinned state
      group.removeAttribute("enso-pinned-changed");
      group.setAttribute("had-enso-pinned-changed", true);
    }
  ...
  async on_TabSelect(event) {
    const tab = gEnsoGlanceManager.getTabOrGlanceParent(event.target);
    let group = tab?.group;
    if (group?.hasAttribute("split-view-group")) {
      group = group?.group;
    }
    if (!group?.isEnsoFolder) {
      return;
    }
  ...
  on_TabOpen(event) {
    ...
    const group = tab.group;
    if (!group?.isEnsoFolder || tab.pinned) {
      return;
    }
    ...
    if (Services.prefs.getBoolPref("enso.folders.owned-tabs-in-folder")) {
      gBrowser.pinTab(tab);
      group.addTabs([tab]);
    }
  }
  ...
  async on_TabUngrouped(event) {
    ...
    if (group.hasAttribute("split-view-group") && tab.hasAttribute("had-enso-pinned-changed")) {
  ...
  #onNewFolder(event) {
    const isFromToolbar = event.target.id === "enso-context-menu-new-folder-toolbar";
    ...
    const canInsertBefore =
      !isFromToolbar &&
      !triggerTab.hasAttribute("enso-essential") &&
      !triggerTab?.group?.hasAttribute("split-view-group") &&
      this.canDropElement({ isEnsoFolder: true }, triggerTab);
  ...
  async #convertFolderToSpace(folder) {
    const currentWorkspace = gEnsoWorkspaces.getActiveWorkspaceFromCache();
    let selectedTab = folder.tabs.find((tab) => tab.selected);
    const icon = folder.icon?.querySelector("svg .icon image");

    const newSpace = await gEnsoWorkspaces.createAndSaveWorkspace(
      folder.label,
        ...
        beforeChangeCallback: async (newWorkspace) => {
          await new Promise((resolve) => {
            requestAnimationFrame(async () => {
              const workspacePinnedContainer = gEnsoWorkspaces.workspaceElement(
                newWorkspace.uuid
              ).pinnedTabsContainer;
              const tabs = folder.allItems.filter((tab) => !tab.hasAttribute("enso-empty-tab"));
              workspacePinnedContainer.append(...tabs);
              await folder.delete();
              gBrowser.tabContainer._invalidateCachedTabs();
              if (selectedTab) {
                selectedTab.setAttribute("enso-workspace-id", newWorkspace.uuid);
                selectedTab.removeAttribute("folder-active");
                gEnsoWorkspaces.lastSelectedWorkspaceTabs[newWorkspace.uuid] = selectedTab;
              }
              resolve();
            });
          });
    ...
    for (const tab of gBrowser.tabs) {
      if (!tab.hasAttribute("enso-essential")) {
        tab.setAttribute("enso-workspace-id", newSpace.uuid);
        tab.style.opacity = "";
        tab.style.height = "";
      }
      gBrowser.TabStateFlusher.flush(tab.linkedBrowser);
      if (gEnsoWorkspaces.lastSelectedWorkspaceTabs[currentWorkspace.uuid] === tab) {
        // This tab is no longer the last selected tab in the previous workspace because it's being moved to
        // the current workspace
        delete gEnsoWorkspaces.lastSelectedWorkspaceTabs[currentWorkspace.uuid];
      }
    }
  }

  changeFolderToSpace(folder, workspaceId, { hasDndSwitch = false } = {}) {
    if (folder.getAttribute("enso-workspace-id") == workspaceId) {
      return;
    }

    const workspaceElement = gEnsoWorkspaces.workspaceElement(workspaceId);
    ...
    if (!hasDndSwitch) {
      gEnsoWorkspaces.changeWorkspaceWithID(workspaceId).then(() => {
  ...
  canDropElement(element, targetElement) {
    const isEnsoFolder = element?.isEnsoFolder;
    const level = targetElement?.group?.level + 1;
    return !(isEnsoFolder && level >= this.#ENSO_MAX_SUBFOLDERS);
  }

  createFolder(tabs = [], options = {}) {
    const filteredTabs = tabs
      .filter((tab) => !tab.hasAttribute("enso-essential"))
      .map((tab) => {
        gBrowser.pinTab(tab);
    ...
    const workspacePinned = gEnsoWorkspaces.workspaceElement(
      options.workspaceId
    )?.pinnedTabsContainer;
    const pinnedContainer =
      options.workspaceId && workspacePinned ? workspacePinned : gEnsoWorkspaces.pinnedTabsContainer;
    const insertBefore =
    ...
    gEnsoVerticalTabsManager.animateItemOpen(folder);
  ...
  _createFolderNode(options = {}) {
    const folder = document.createXULElement("enso-folder", { is: "enso-folder" });
    let id = options.id;
    if (!id) {
    ...
    folder.color = "enso-workspace-color";

    folder.setAttribute("enso-workspace-id", options.workspaceId || gEnsoWorkspaces.activeWorkspace);
  ...
  handleTabPin(tab) {
    ...
    if (group.hasAttribute("split-view-group") && !this._piningFolder) {
      ...
      for (const otherTab of group.tabs) {
        gEnsoPinnedTabManager.resetPinChangedUrl(otherTab);
        if (tab === otherTab) {
          continue;
        }
  ...
  handleTabUnpin(tab) {
    tab.style.removeProperty("--enso-folder-indent");
    const group = tab.group;
    if (!group) {
      return false;
    }
    if (group.hasAttribute("split-view-group") && !this._piningFolder) {
      ...
      this._piningFolder = false;
      gEnsoWorkspaces.activeWorkspaceStrip.prepend(group);
      gBrowser.tabContainer._invalidateCachedTabs();
      return true;
    }
    return this._piningFolder;
  ...
  openTabsPopup(event) {
    ...
    if (document.documentElement.getAttribute("enso-renaming-tab") || gURLBar.focused) {
      return;
    }
    ...
    document.getElementById("enso-folder-tabs-search-no-results").hidden = true;
    ...
    const onSearchInput = () => {
      ...
      document.getElementById("enso-folder-tabs-search-no-results").hidden = foundTabs > 0;
    };
  ...
  get #searchPopupOptions() {
    const isRightSide = gEnsoVerticalTabsManager._prefsRightSide;
  ...
  #shouldAppearOnTabSearch(tab, group) {
    ...
    return !tabIsInActiveGroup && !(tab.hidden || tab.hasAttribute("enso-empty-tab"));
  }
  ...
  setFolderIndentation(tabs, groupElem = undefined, forCollapse = true, animate = true) {
    ...
    const isSpaceCollapsed = gEnsoWorkspaces.activeWorkspaceElement?.hasCollapsedPinnedTabs;
    ...
    if (
      ...
      (!(groupElem.hasAttribute("enso-empty-tab") && groupElem.group === tab.group) ||
        groupElem?.hasAttribute("enso-empty-tab"))
    ) {
    ...
    for (const tabItem of tabs) {
      if (gBrowser.isTabGroupLabel(tabItem) || tabItem.group?.hasAttribute("split-view-group")) {
        tabItem.group.style.setProperty("--enso-folder-indent", `${spacing}px`);
        continue;
      }
      tabItem.style.setProperty("--enso-folder-indent", `${spacing}px`);
    }
    if (!animate) {
      for (const tabItem of tabs) {
  ...
  #groupInit(group, stateData) {
    // Setup enso-folder icon to the correct position
    ...
    labelContainer.addEventListener("mouseenter", (event) => {
      if (
        !group.collapsed ||
        !Services.prefs.getBoolPref("enso.folders.search.enabled") ||
      ...
      }, Services.prefs.getIntPref("enso.folders.search.hover-delay"));
    });
  ...
  storeDataForSessionStore() {
    const folders = Array.from(gBrowser.tabContainer.querySelectorAll("enso-folder"));
    ...
    for (const folder of allData) {
      const parentFolder = folder.parentElement.closest("enso-folder");
      // Skip split-view-group if it's not a enso-folder child
      if (!parentFolder && folder.hasAttribute("split-view-group")) {
        continue;
      }
      const emptyFolderTabs = folder.tabs
        .filter((tab) => tab.hasAttribute("enso-empty-tab"))
      ...
      storedData.push({
        ...
        workspaceId: folder.getAttribute("enso-workspace-id"),
      });
    }
    return storedData;
  }
    ...
    for (const folderData of data) {
      try {
        ...
        folderData.emptyTabIds.forEach((id) => {
          oldGroup?.querySelector(`tab[id="${id}"]`)?.setAttribute("enso-empty-tab", true);
        });
        if (gBrowser.isTabGroup(oldGroup)) {
          ...
          while (oldGroup.tabs.length) {
            ...
            if (folderData.workspaceId) {
              tab.setAttribute("enso-workspace-id", folderData.workspaceId);
            }
        ...
        console.error("Error restoring Enso Folders session data:", e);
      }
    }
        ...
        if (parentWorkingData && parentWorkingData.node) {
          switch (stateData?.prevSiblingInfo?.type) {
            ...
            case "group": {
              ...
              console.warn(
                `Enso Folders: Could not find previous sibling group with id ${stateData.prevSiblingInfo.id} while restoring session.`
              );
              // @eslint-disable-next-line no-fallthrough
            }
            default: {
              // Should insert after enso-empty-tab
  ...
  highlightGroupOnDragOver(folder, movingTabs = null) {
    ...
    if (
      ...
      !(
        folder.level >= this.#ENSO_MAX_SUBFOLDERS &&
  ...
  #normalizeGroupItems(items) {
    return items
      .filter((item) => !item.hasAttribute("enso-empty-tab"))
      .map((item) => {
  ...
  #createAnimation(items, targetState, opts, callback = () => {}) {
    ...
    return items.map((item) =>
      gEnsoUIManager.motion.animate(item, targetState, opts).then(callback)
    );
  }
      ...
      if (group.hasAttribute("has-active")) {
        ...
        for (let tab of activeTabs) {
          ...
          if (activeGroup) {
            ...
            if (tab.group?.hasAttribute("split-view-group")) {
              tab.group.style.removeProperty("--enso-folder-indent");
            } else {
              tab.style.removeProperty("--enso-folder-indent");
            }
          }
  ...
  async animateUnloadAll(group) {
    ...
    for (const folder of activeGroups) {
      ...
      const afterAnimate = () => {
        ...
        // Trigger the recalculation so that enso returns
  ...
  async animateUnload(group, tabToUnload, ungroup = false) {
    ...
    if ((!group?.isEnsoFolder || !isSplitView) && !tabToUnload.hasAttribute("folder-active")) {
      return;
    }
    const animations = [];
    let lastTab = false;
    ...
    tabToUnload.style.removeProperty("--enso-folder-indent");
...
window.gEnsoFolders = new nsEnsoFolders();
