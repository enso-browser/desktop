...
import { nsZenFolder } from "chrome://browser/content/enso-components/ZenFolder.mjs";
...
export class nsZenWorkspace extends MozXULElement {
  ...
  static get markup() {
    return `
        <vbox class="enso-workspace-tabs-section enso-current-workspace-indicator enso-drop-target" flex="1" context="ensoWorkspaceMoreActions">
          <stack class="enso-current-workspace-indicator-stack">
            <image class="enso-current-workspace-indicator-chevron" />
            <hbox class="enso-current-workspace-indicator-icon" />
          </stack>
          <label class="enso-current-workspace-indicator-name" />
          <toolbarbutton class="toolbarbutton-1 chromeclass-toolbar-additional enso-workspaces-actions" context="ensoWorkspaceMoreActions" />
        </vbox>
        <arrowscrollbox orient="vertical" class="workspace-arrowscrollbox">
          <vbox class="enso-workspace-tabs-section enso-workspace-pinned-tabs-section" hide-separator="true">
            <html:div class="enso-tab-group-start space-fake-collapsible-start" style="order: -9999;" />
            <hbox class="pinned-tabs-container-separator">
              <toolbarseparator flex="1" />
              <toolbarbutton command="cmd_ensoCloseUnpinnedTabs"
                             tooltip="dynamic-shortcut-tooltip"
                             data-l10n-id="enso-workspaces-close-all-unpinned-tabs-title"
                             class="enso-workspace-close-unpinned-tabs-button" />
            </hbox>
          </vbox>
          <vbox class="enso-workspace-tabs-section enso-workspace-normal-tabs-section">
        ...
        <vbox class="enso-workspace-empty-space" flex="1" />
      `;
  }

  static get moveTabToButtonMarkup() {
    return `
      <toolbarbutton class="toolbarbutton-1 chromeclass-toolbar-additional enso-workspaces-actions"
                     tooltip="dynamic-shortcut-tooltip"
                     data-l10n-id="enso-move-tab-to-workspace-button" />
    `;
  }

  static get inheritedAttributes() {
    return {
      ".enso-workspace-tabs-section": "enso-workspace-id=id",
    };
  }
  ...
  connectedCallback() {
    ...
    this.tabsContainer = this.querySelector(".enso-workspace-normal-tabs-section");
    this.indicator = this.querySelector(".enso-current-workspace-indicator");
    this.pinnedTabsContainer = this.querySelector(".enso-workspace-pinned-tabs-section");
    ...
    const indicatorName = this.indicator.querySelector(".enso-current-workspace-indicator-name");
    ...
    this.indicator
      .querySelector(".enso-workspaces-actions")
      .addEventListener("click", this.onActionsCommand.bind(this));

    this.indicator
      .querySelector(".enso-current-workspace-indicator-icon")
    ...
    if (!gEnsoWorkspaces.currentWindowIsSyncing) {
      let actionsButton = this.indicator.querySelector(".enso-workspaces-actions");
    ...
    this.scrollbox._getScrollableElements = () => {
      ...
      if (Services.prefs.getBoolPref("enso.view.show-newtab-button-top", false)) {
    ...
    this.scrollbox._canScrollToElement = (element) => {
      if (gBrowser.isTab(element)) {
        return (
          !element.hasAttribute("enso-essential") &&
          !this.hasAttribute("positionpinnedtabs") &&
          !element.hasAttribute("enso-empty-tab")
        );
      }
      return true;
    };
    ...
    this.tabsContainer.setAttribute("enso-workspace-id", this.id);
    this.pinnedTabsContainer.setAttribute("enso-workspace-id", this.id);

    this.collapsiblePins = document.createXULElement("enso-workspace-collapsible-pins");
  ...
  async onIndicatorRenameFinished(newName) {
    ...
    let workspaces = gEnsoWorkspaces.getWorkspaces();
    let workspaceData = workspaces.find((workspace) => workspace.uuid === this.workspaceUuid);
    workspaceData.name = newName;
    await gEnsoWorkspaces.saveWorkspace(workspaceData);
    this.indicator.querySelector(".enso-current-workspace-indicator-name").textContent = newName;
    gEnsoUIManager.showToast("enso-workspace-renamed-toast");
  }

  onActionsCommand(event) {
    event.stopPropagation();
    const popup = document.getElementById("ensoWorkspaceMoreActions");
  ...
  #onGradientCacheChanged() {
    const { isDarkMode, isExplicitMode, toolbarColor, primaryColor } =
      gEnsoThemePicker.getGradientForWorkspace(
        gEnsoWorkspaces.getWorkspaceFromId(this.workspaceUuid)
      );
    if (isExplicitMode) {
    ...
    this.style.setProperty("--enso-primary-color", primaryColor);
  }
  ...
  clearThemeStyles() {
    ...
    this.style.removeProperty("--enso-primary-color");
  }
  ...
  #openMoveTabsToWorkspacePanel(button) {
    ...
    const popup = document.getElementById("ensoMoveTabsToSyncedWorkspacePopup");
    popup.innerHTML = "";

    const workspaces = gEnsoWorkspaces.getWorkspaces(true);
    for (const workspace of workspaces) {
      const item = gEnsoWorkspaces.generateMenuItemForWorkspace(workspace);
      item.addEventListener("command", async () => {
        const { EnsoWindowSync } = ChromeUtils.importESModule(
          "resource:///modules/enso/EnsoWindowSync.sys.mjs"
        );
        EnsoWindowSync.moveTabsToSyncedWorkspace(window, workspace.uuid);
      });
...
customElements.define("enso-workspace", nsZenWorkspace);
customElements.define("enso-workspace-collapsible-pins", nsZenCollapsiblePins);
