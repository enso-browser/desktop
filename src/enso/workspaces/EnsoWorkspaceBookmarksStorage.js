...
window.EnsoWorkspaceBookmarksStorage = {
  lazy: {},
  ...
  async _ensureTable() {
    await this.lazy.PlacesUtils.withConnectionWrapper(
      "EnsoWorkspaceBookmarksStorage.init",
      async (db) => {
        // Create table using GUIDs instead of IDs
        await db.execute(`
        CREATE TABLE IF NOT EXISTS enso_bookmarks_workspaces (
      ...
      `);
        ...
        CREATE INDEX IF NOT EXISTS idx_bookmarks_workspaces_lookup
          ON enso_bookmarks_workspaces(workspace_uuid, bookmark_guid)
      `);

        // Add changes tracking table
        await db.execute(`
        CREATE TABLE IF NOT EXISTS enso_bookmarks_workspaces_changes (
          id INTEGER PRIMARY KEY,
          bookmark_guid TEXT NOT NULL,
          workspace_uuid TEXT NOT NULL,
      ...
      `);
        ...
        CREATE INDEX IF NOT EXISTS idx_bookmarks_workspaces_changes
          ON enso_bookmarks_workspaces_changes(bookmark_guid, workspace_uuid)
      `);
      }
    );
  },
  ...
  async updateLastChangeTimestamp(db) {
    ...
    await db.execute(
      ...
      VALUES ('enso_bookmarks_workspaces_last_change', :now)
    `,
      { now }
    );
  },
  ...
  async getLastChangeTimestamp() {
    ...
    const result = await db.executeCached(`
      SELECT value FROM moz_meta WHERE key = 'enso_bookmarks_workspaces_last_change'
    `);
  ...
  async getBookmarkWorkspaces(bookmarkGuid) {
    ...
    const rows = await db.execute(
      ...
      FROM enso_bookmarks_workspaces
  ...
  async getChangedIDs() {
    ...
    const rows = await db.execute(`
      ...
      FROM enso_bookmarks_workspaces_changes
    `);
  ...
  async clearChangedIDs() {
    await this.lazy.PlacesUtils.withConnectionWrapper(
      "EnsoWorkspaceBookmarksStorage.clearChangedIDs",
      async (db) => {
        await db.execute(`DELETE FROM enso_bookmarks_workspaces_changes`);
      }
    );
  },
};
