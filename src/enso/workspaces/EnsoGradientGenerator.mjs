...
ChromeUtils.defineLazyGetter(lazy, "MAX_OPACITY", () => {
  return parseFloat(document.getElementById("PanelUI-enso-gradient-generator-opacity").max);
});

ChromeUtils.defineLazyGetter(lazy, "MIN_OPACITY", () => {
  return parseFloat(document.getElementById("PanelUI-enso-gradient-generator-opacity").min);
});
...
/**
 * A class that manages the theme picker for Enso Workspaces.
 * It allows users to create and customize gradients for their workspace themes.
 */
export class nsEnsoThemePicker extends nsZenMultiWindowFeature {
  static MAX_DOTS = 3;
  ...
  #allowTransparencyOnSidebar = Services.prefs.getBoolPref("enso.theme.acrylic-elements", false);
  ...
  constructor() {
    super();
    if (!gEnsoWorkspaces.shouldHaveWorkspaces || gEnsoWorkspaces.privateWindowOrDisabled) {
      return;
    }
    ...
    this.isLegacyVersion = Services.prefs.getIntPref("enso.theme.gradient-legacy-version", 1) === 0;

    ChromeUtils.defineLazyGetter(this, "panel", () =>
      document.getElementById("PanelUI-enso-gradient-generator")
    );
    ...
    ChromeUtils.defineLazyGetter(this, "customColorInput", () =>
      document.getElementById("PanelUI-enso-gradient-generator-custom-input")
    );
    ChromeUtils.defineLazyGetter(this, "customColorList", () =>
      document.getElementById("PanelUI-enso-gradient-generator-custom-list")
    );

    ChromeUtils.defineLazyGetter(this, "sliderWavePath", () =>
      document.getElementById("PanelUI-enso-gradient-slider-wave").querySelector("path")
    );
    ...
    document
      .getElementById("PanelUI-enso-gradient-generator-opacity")
    ...
    XPCOMUtils.defineLazyPreferenceGetter(
      this,
      "windowSchemeType",
      "enso.view.window.scheme",
      2,
      darkModeChange
    );

    XPCOMUtils.defineLazyPreferenceGetter(this, "darkModeBias", "enso.theme.dark-mode-bias", "0.5");
  }
  ...
  initContextMenu() {
    const menu = window.MozXULElement.parseXULToFragment(`
        <menuitem id="ensoToolbarThemePicker"
                  data-lazy-l10n-id="enso-workspaces-change-theme"
  ...
  openThemePicker(event) {
    const fromForm = event.explicitOriginalTarget?.classList?.contains(
      "enso-workspace-creation-edit-theme-button"
    );
  ...
  initPredefinedColors() {
    document
      .getElementById("PanelUI-enso-gradient-generator-color-pages")
  ...
  initColorPages() {
    const leftButton = document.getElementById("PanelUI-enso-gradient-generator-color-page-left");
    const rightButton = document.getElementById("PanelUI-enso-gradient-generator-color-page-right");
    const pagesWrapper = document.getElementById("PanelUI-enso-gradient-generator-color-pages");
  ...
  initSchemeButtons() {
    const buttons = document.getElementById("PanelUI-enso-gradient-generator-scheme");
    buttons.addEventListener("click", (event) => {
      ...
      event.preventDefault();
      event.stopPropagation();
      const scheme = target.id.replace("PanelUI-enso-gradient-generator-scheme-", "");
      if (!scheme) {
        return;
      }
      const themeInt = {
      ...
      Services.prefs.setIntPref("enso.view.window.scheme", themeInt);
    });
  }

  initTextureInput() {
    const wrapper = document.getElementById("PanelUI-enso-gradient-generator-texture-wrapper");
    const wrapperWidth = wrapper.getBoundingClientRect().width;
    // Add elements in a circular pattern, where the center is the center of the wrapper
    for (let i = 0; i < 16; i++) {
      const dot = document.createElement("div");
      dot.classList.add("enso-theme-picker-texture-dot");
    ...
    this._textureHandler.id = "PanelUI-enso-gradient-generator-texture-handler";
  ...
  initThemePicker() {
    const themePicker = this.panel.querySelector(".enso-theme-picker-gradient");
  ...
  getColorFromPosition(x, y, type = undefined) {
    ...
    const gradient = this.panel.querySelector(".enso-theme-picker-gradient");
  ...
  createDot(color, fromWorkspace = false) {
    ...
    if (color.isCustom) {
      ...
      dot.style.opacity = 0;
      dot.style.setProperty("--enso-theme-picker-dot-color", color.c);
    } else {
      const { x, y } = color.position || this.calculateInitialPosition([r, g, b]);
      const dotPad = this.panel.querySelector(".enso-theme-picker-gradient");

      dot.classList.add("enso-theme-picker-dot");
      ...
      dot.style.setProperty("--enso-theme-picker-dot-color", `rgb(${r}, ${g}, ${b})`);
  ...
  addColorToCustomList(color) {
    const listItems = window.MozXULElement.parseXULToFragment(`
        <hbox class="enso-theme-picker-custom-list-item">
          <html:div class="enso-theme-picker-dot custom"></html:div>
          <label class="enso-theme-picker-custom-list-item-label"></label>
          <toolbarbutton class="enso-theme-picker-custom-list-item-remove toolbarbutton-1"></toolbarbutton>
        </hbox>
      `);
    listItems.querySelector(".enso-theme-picker-custom-list-item").setAttribute("data-color", color);
    listItems
      .querySelector(".enso-theme-picker-dot")
      .style.setProperty("--enso-theme-picker-dot-color", color);
    listItems.querySelector(".enso-theme-picker-custom-list-item-label").textContent = color;
    listItems
      .querySelector(".enso-theme-picker-custom-list-item-remove")
  ...
  async addCustomColor() {
    ...
    let colorOpacity =
      document.getElementById("PanelUI-enso-gradient-generator-custom-opacity")?.value ?? 1;
    ...
    const dot = document.createElement("div");
    dot.classList.add("enso-theme-picker-dot", "hidden", "custom");
    dot.style.opacity = 0;
    dot.style.setProperty("--enso-theme-picker-dot-color", color);
    this.panel.querySelector("#PanelUI-enso-gradient-generator-custom-list").prepend(dot);
  ...
  handlePanelCommand(event) {
    ...
    switch (target.id) {
      case "PanelUI-enso-gradient-generator-color-custom-add":
        this.addCustomColor();
        break;
    }
  }
  ...
  handleColorPositions(colorPositions, ignoreLegacy = false) {
    ...
    if (this.isLegacyVersion && !ignoreLegacy) {
      ...
      Services.prefs.setIntPref("enso.theme.gradient-legacy-version", 1);
    }
  ...
  onThemePickerClick(event) {
    ...
    if (event.target.closest("#PanelUI-enso-gradient-generator-scheme")) {
      return;
    }
    event.preventDefault();
    const target = event.target;
    if (target.id === "PanelUI-enso-gradient-generator-color-add") {
      if (this.dots.length >= nsEnsoThemePicker.MAX_DOTS) {
        return;
      }
      ...
      this.updateCurrentWorkspace();
      return;
    } else if (target.id === "PanelUI-enso-gradient-generator-color-remove") {
      ...
      this.updateCurrentWorkspace();
      return;
    } else if (target.id === "PanelUI-enso-gradient-generator-color-toggle-algo") {
  ...
  onDotMouseUp(event) {
    ...
    if (event.button === 2) {
      if (!event.target.classList.contains("enso-theme-picker-dot")) {
        return;
      }
  ...
  onDotMouseMove(event) {
    if (this.dragging) {
      event.preventDefault();
      const rect = this.panel.querySelector(".enso-theme-picker-gradient").getBoundingClientRect();
  ...
  get canBeTransparent() {
    return (
      this.isMica ||
      window.matchMedia(
        '(-moz-platform: macos) or ((-moz-platform: linux) and -moz-pref("enso.widget.linux.transparency"))'
      ).matches
    );
  }
  ...
  shouldBeDarkMode(accentColor) {
    if (Services.prefs.getBoolPref("enso.theme.use-system-colors")) {
  ...
  updateNoise(texture) {
    document.documentElement.style.setProperty("--enso-grainy-background-opacity", texture);
    document.documentElement.setAttribute(
      "enso-show-grainy-background",
  ...
  onWorkspaceChange(workspace, skipUpdate = false, theme = null) {
    ...
    this.forEachWindowSync((browser) => {
      if (!browser.gEnsoThemePicker?.promiseInitialized) {
        return;
      }
      ...
      if (theme === null) {
        browser.gEnsoThemePicker.invalidateGradientCache();
      }

      // Do not rebuild if the workspace is not the same as the current one
      const windowWorkspace = browser.gEnsoWorkspaces.getActiveWorkspace();
      ...
      if (!skipUpdate) {
        for (const dot of browser.gEnsoThemePicker.panel.querySelectorAll(".enso-theme-picker-dot")) {
          dot.remove();
        }
      }

      if (theme) {
        const workspaceElement = browser.gEnsoWorkspaces.workspaceElement(windowWorkspace.uuid);
        if (workspaceElement) {
      ...
      if (!skipUpdate) {
        docElement.style.setProperty(
          ...
          docElement.style.getPropertyValue("--enso-main-browser-background")
        );
        docElement.style.setProperty(
          "--enso-main-browser-background-toolbar-old",
          docElement.style.getPropertyValue("--enso-main-browser-background-toolbar")
        );
        docElement.style.setProperty(
          "--enso-background-opacity",
          browser.gEnsoThemePicker.previousBackgroundOpacity ?? 1
        );
        if (browser.gEnsoThemePicker.previousBackgroundResolve) {
          browser.gEnsoThemePicker.previousBackgroundResolve();
        }
        delete browser.gEnsoThemePicker.previousBackgroundOpacity;
      }

      browser.gEnsoThemePicker.resetCustomColorList();

      browser.gEnsoThemePicker.currentOpacity = workspaceTheme.opacity ?? 0.5;
      browser.gEnsoThemePicker.currentTexture = workspaceTheme.texture ?? 0;
      ...
      const opacitySlider = browser.document.getElementById(
        "PanelUI-enso-gradient-generator-opacity"
      );

      {
        let opacity = browser.gEnsoThemePicker.currentOpacity;
        const svg = browser.gEnsoThemePicker.sliderWavePath;
        /* eslint-disable no-unused-vars */
        const [_, secondStop, thirdStop] = document.querySelectorAll(
          "#PanelUI-enso-gradient-generator-slider-wave-gradient stop"
        );
        ...
        opacitySlider.style.setProperty("--enso-thumb-height", `${40 + opacity * 15}px`);
        opacitySlider.style.setProperty("--enso-thumb-width", `${10 + opacity * 15}px`);
        svg.style.stroke =
          interpolatedPath === this.#linePath
            ? thirdStop.getAttribute("stop-color")
            : "url(#PanelUI-enso-gradient-generator-slider-wave-gradient)";
      }

      for (const button of browser.document.querySelectorAll(
        "#PanelUI-enso-gradient-generator-color-actions button"
      )) {
        // disable if there are no buttons
        button.disabled =
          workspaceTheme.gradientColors.length === 0 ||
          (button.id === "PanelUI-enso-gradient-generator-color-add"
            ? workspaceTheme.gradientColors.length >= nsEnsoThemePicker.MAX_DOTS
            : false) ||
          (button.id === "PanelUI-enso-gradient-generator-color-toggle-algo"
            ? workspaceTheme.gradientColors.length < 2
            : false);
      }
      const clickToAdd = browser.document.getElementById(
        "PanelUI-enso-gradient-generator-color-click-to-add"
      );
      ...
      opacitySlider.value = browser.gEnsoThemePicker.currentOpacity;
      const textureSelectWrapper = browser.document.getElementById(
        "PanelUI-enso-gradient-generator-texture-wrapper"
      );
      ...
      if (textureWrapperWidth) {
        ...
        let textureHandler = browser.gEnsoThemePicker._textureHandler;
        ...
        let buttons = browser.document.querySelectorAll(".enso-theme-picker-texture-dot");
        let i = 4;
      ...
      const gradient = browser.gEnsoThemePicker.getGradient(workspaceTheme.gradientColors);
      const gradientToolbar = browser.gEnsoThemePicker.getGradient(
        workspaceTheme.gradientColors,
        true
      );
      browser.gEnsoThemePicker.updateNoise(workspaceTheme.texture);

      browser.gEnsoThemePicker.customColorList.innerHTML = "";
      for (const dot of workspaceTheme.gradientColors) {
        if (dot.isCustom) {
          browser.gEnsoThemePicker.addColorToCustomList(dot.c);
        }
      }

      docElement.style.setProperty("--enso-main-browser-background-toolbar", gradientToolbar);
      docElement.style.setProperty("--enso-main-browser-background", gradient);
      const isDarkModeWindow = browser.gEnsoThemePicker.isDarkMode;
      if (isDefaultTheme) {
        docElement.setAttribute("enso-default-theme", "true");
      } else {
        docElement.removeAttribute("enso-default-theme");
      }
      if (dominantColor) {
        const primaryColor = this.getAccentColorForUI(dominantColor);
        docElement.style.setProperty("--enso-primary-color", primaryColor);
        ...
        browser.gEnsoThemePicker.isLegacyVersion =
          Services.prefs.getIntPref("enso.theme.gradient-legacy-version", 1) === 0;

        let isDarkMode = isDarkModeWindow;
        if (!isDefaultTheme && !this.isLegacyVersion) {
          // Check for the primary color
          isDarkMode = browser.gEnsoThemePicker.shouldBeDarkMode(dominantColor);
          docElement.setAttribute("enso-should-be-dark-mode", isDarkMode);
          browser.gEnsoThemePicker.panel.removeAttribute("invalidate-controls");
        } else {
          docElement.removeAttribute("enso-should-be-dark-mode");
          if (!this.isLegacyVersion) {
            browser.gEnsoThemePicker.panel.setAttribute("invalidate-controls", "true");
          }
        }
      ...
      if (!skipUpdate) {
        browser.gEnsoThemePicker.dots = [];
        browser.gEnsoThemePicker.recalculateDots(workspaceTheme.gradientColors);
      }
    });
  }
  ...
  getNativeAccentColor() {
    let accentColor = Services.prefs.getStringPref("enso.theme.accent-color");
    let rgb;
  ...
  removeCustomColor(event) {
    const target = event.target.closest(".enso-theme-picker-custom-list-item");
    const color = target.getAttribute("data-color");
    const dots = this.panel.querySelectorAll(".enso-theme-picker-dot");
    for (const dot of dots) {
      if (dot.style.getPropertyValue("--enso-theme-picker-dot-color") === color) {
        dot.remove();
        break;
      }
    }
  ...
  updateCurrentWorkspace(skipSave = true) {
    ...
    const colors = Array.from(dots)
      ...
      .map((dot) => {
        const color = dot.style.getPropertyValue("--enso-theme-picker-dot-color");
    ...
    const gradient = nsEnsoThemePicker.getTheme(colors, this.currentOpacity, this.currentTexture);
    let currentWorkspace = gEnsoWorkspaces.getActiveWorkspace();

    if (!skipSave) {
      currentWorkspace.theme = gradient;
      gEnsoWorkspaces.saveWorkspace(currentWorkspace);
    }
