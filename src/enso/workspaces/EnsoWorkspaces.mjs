...
import { nsEnsoThemePicker } from "chrome://browser/content/enso-components/EnsoGradientGenerator.mjs";

const lazy = {};

ChromeUtils.defineESModuleGetters(lazy, {
  EnsoSessionStore: "resource:///modules/enso/EnsoSessionManager.sys.mjs",
});

/**
 * Enso Spaces manager. This class is mainly responsible for the UI
 * and user interactions but it also contains some logic to manage
 * the workspaces and their tabs.
 *
 * For window sync, please @see EnsoWindowSync
 */
class nsEnsoWorkspaces {
  /**
  ...
  #canDebug = Services.prefs.getBoolPref("enso.workspaces.debug", false);
  #activeWorkspace = "";
  ...
  async init() {
    ...
    if (!this.shouldHaveWorkspaces) {
      ...
      console.warn("gEnsoWorkspaces: !!! gEnsoWorkspaces is disabled in hidden windows !!!");
      return; // We are in a hidden window, don't initialize gEnsoWorkspaces
    }
    ...
    XPCOMUtils.defineLazyPreferenceGetter(
      ...
      "enso.workspaces.force-container-workspace",
      true
    );
    XPCOMUtils.defineLazyPreferenceGetter(
      this,
    ...
    ChromeUtils.defineLazyGetter(this, "workspaceIcons", () =>
      document.getElementById("enso-workspaces-button")
    );
    this.#activeWorkspace ||= Services.prefs.getStringPref("enso.workspaces.active", "");

    if (this.isPrivateWindow) {
      document.documentElement.setAttribute("enso-private-window", "true");
    }
  ...
  log(...args) {
    if (this.#canDebug) {
      ...
      console.debug(`[gEnsoWorkspaces]:`, ...args);
    }
  }
  ...
  async #afterLoadInit() {
    ...
    if (
      Services.prefs.getBoolPref("enso.workspaces.swipe-actions", false) &&
  ...
  async selectEmptyTab(newTabTarget = null) {
    ...
    if (gEnsoUIManager.testingEnabled) {
      return null;
    }
    ...
    try {
      ...
      if (
        ...
        gEnsoVerticalTabsManager._canReplaceNewTab
      ) {
      ...
      let tab = gEnsoUIManager.openAndChangeToTab(newTabUrl);

      // Set workspace ID if available
      if (window.uuid) {
        tab.setAttribute("enso-workspace-id", this.activeWorkspace);
      }
      return tab;
    } catch (e) {
  ...
  #initializeEmptyTab() {
    for (const tab of gBrowser.tabs) {
      ...
      if (tab.hasAttribute("enso-empty-tab") && !tab.pinned) {
  ...
  registerPinnedResizeObserver() {
    ...
    for (let element of document.getElementById("enso-essentials").children) {
  ...
  get tabboxChildrenWithoutEmpty() {
    return this.tabboxChildren.filter((child) => !child.hasAttribute("enso-empty-tab"));
  }

  get shouldAnimateEssentials() {
    return (
      this.containerSpecificEssentials ||
      document.documentElement.hasAttribute("enso-creating-workspace")
    );
  }
  ...
  getEssentialsSection(container = 0) {
    ...
    let essentialsContainer = document.querySelector(
      `.enso-essentials-container[container="${container}"]:not([cloned])`
    );
    if (!essentialsContainer) {
      essentialsContainer = document.createXULElement("hbox");
      essentialsContainer.className = "enso-essentials-container enso-workspace-tabs-section";
      essentialsContainer.setAttribute("flex", "1");
      essentialsContainer.setAttribute("container", container);
      document.getElementById("enso-essentials").appendChild(essentialsContainer);
  ...
  #createWorkspaceTabsSection(workspace, tabs = []) {
    const workspaceWrapper = document.createXULElement("enso-workspace");
    ...
    if (document.documentElement.hasAttribute("enso-creating-workspace")) {
  ...
  #organizeTabsToWorkspaceSections(workspace, section, pinnedSection, tabs) {
    const workspaceTabs = Array.from(tabs).filter(
      (tab) =>
        tab.getAttribute("enso-workspace-id") === workspace.uuid &&
        !tab.hasAttribute("enso-essential")
    );
    let folders = new Set();
  ...
  get _hoveringSidebar() {
    return gNavToolbox.hasAttribute("enso-has-implicit-hover");
  }
  ...
  attachGestureHandlers(element) {
    ...
    element.addEventListener(
      "MozSwipeGestureEnd",
      () => {
        Services.prefs.setBoolPref("enso.swipe.is-fast-swipe", false);
        document.documentElement.removeAttribute("swipe-gesture");
        gEnsoUIManager.tabsWrapper.style.removeProperty("scrollbar-width");
        document.documentElement.style.setProperty("--enso-background-opacity", "1");
  ...
  _handleSwipeMayStart(event) {
    ...
    if (
      event.target.closest("#enso-sidebar-foot-buttons") ||
      event.target.closest('#urlbar[enso-floating-urlbar="true"]')
    ) {
      return;
    }
  ...
  _handleSwipeStart(event) {
    ...
    Services.prefs.setBoolPref("enso.swipe.is-fast-swipe", true);
  }
  ...
  _handleSwipeUpdate(event) {
    ...
    const stripWidth =
      window.windowUtils.getBoundsWithoutFlushing(document.getElementById("navigator-toolbox"))
        .width +
      window.windowUtils.getBoundsWithoutFlushing(document.getElementById("enso-sidebar-splitter"))
        .width *
        2;
  ...
  set activeWorkspace(value) {
    ...
    Services.prefs.setStringPref("enso.workspaces.active", value);
  }
  ...
  get currentWindowIsSyncing() {
    return (
      !document.documentElement.hasAttribute("enso-unsynced-window") &&
      window._ensoStartupSyncFlag !== "unsynced" &&
      !this.isPrivateWindow
    );
  }
  ...
  getWorkspaces(lieToMe = false) {
    if (lieToMe) {
      const { EnsoSessionStore } = ChromeUtils.importESModule(
        "resource:///modules/enso/EnsoSessionManager.sys.mjs"
      );
      return EnsoSessionStore.getClonedSpaces();
    }
  ...
  async workspaceBookmarks() {
    ...
    const [bookmarks, lastChangeTimestamp] = await Promise.all([
      EnsoWorkspaceBookmarksStorage.getBookmarkGuidsByWorkspace(),
      EnsoWorkspaceBookmarksStorage.getLastChangeTimestamp(),
    ]);
  ...
  restoreWorkspacesFromSessionStore(aWinData = {}) {
    ...
    if (
      ...
      lazy.EnsoSessionStore._migrationData
    ) {
      spacesFromStore.push(...lazy.EnsoSessionStore._migrationData.spaces);
    }
    this._workspaceCache = spacesFromStore.length
      ? [...spacesFromStore]
      : [this.#createWorkspaceData("Space", undefined)];
    this.activeWorkspace = aWinData.activeEnsoSpace || this._workspaceCache[0].uuid;
  ...
  #initializeWorkspaces() {
    ...
    try {
      if (activeWorkspace) {
        window.gEnsoThemePicker = new nsEnsoThemePicker();
        gEnsoThemePicker.onWorkspaceChange(activeWorkspace);
      }
    } catch (e) {
      console.error("gEnsoWorkspaces: Error initializing theme picker", e);
    }
  ...
  async selectStartPage() {
    if (!this.workspaceEnabled || gEnsoUIManager.testingEnabled) {
      return;
    }
    ...
    if (this._tabToRemoveForEmpty && !removedEmptyTab && !this._shouldOverrideTabs) {
      ...
      if (
        ...
        setTimeout(() => {
          let tabToUse = gEnsoGlanceManager.getTabOrGlanceParent(
    ...
    showed &&= Services.prefs.getBoolPref("enso.urlbar.open-on-startup", true);

    // Wait for the next event loop to ensure that the startup focus logic by
    // firefox has finished doing it's thing.
    setTimeout(() => {
      setTimeout(() => {
        if (gEnsoVerticalTabsManager._canReplaceNewTab && showed) {
    ...
    if (
      !gEnsoVerticalTabsManager._canReplaceNewTab &&
      !Services.prefs.getBoolPref("enso.workspaces.continue-where-left-off")
    ) {
      ...
      const tabs = gBrowser.tabs.filter(
        (tab) => !tab.collapsed && !tab.hasAttribute("enso-empty-tab") && !tab.pinned
      );
      for (const tab of tabs) {
  ...
  handleInitialTab(tab, isEmpty) {
    if (gEnsoUIManager.testingEnabled || !this.workspaceEnabled) {
      return;
    }
    // note: We cant access `gEnsoVerticalTabsManager._canReplaceNewTab` this early
    if (isEmpty && Services.prefs.getBoolPref("enso.urlbar.replace-newtab", true)) {
  ...
  changeWorkspaceIcon() {
    let anchor = this.activeWorkspaceIndicator?.querySelector(
      ".enso-current-workspace-indicator-icon"
    );
  ...
  async #clearAnyZombieTabs() {
    ...
    const workspaces = this.getWorkspaces();
    for (let tab of tabs) {
      const workspaceID = tab.getAttribute("enso-workspace-id");
      if (
        (workspaceID &&
          !tab.hasAttribute("enso-essential") &&
        ...
        (tab.pinned && tab.hasAttribute("enso-empty-tab") && !tab.group)
      ) {
  ...
  handleTabBeforeClose(tab, closeWindowWithLastTab) {
    ...
    let workspaceID = tab.getAttribute("enso-workspace-id");
    if (!workspaceID) {
      return null;
    }
  ...
  addPopupListeners() {
    const workspaceActions = document.getElementById("ensoWorkspaceMoreActions");
    ...
    const contextChangeContainerTabMenu = document.getElementById(
      "context_ensoWorkspacesOpenInContainerTab"
    );
  ...
  generateMenuItemForWorkspace(workspace, disableCurrent = false) {
    ...
    item.className = "enso-workspace-context-menu-item";
    item.setAttribute("enso-workspace-id", workspace.uuid);
    if (!disableCurrent) {
    ...
    if (iconIsSvg) {
      ...
      item.classList.add("enso-workspace-context-icon");
    }
    return item;
  }
  ...
  updateWorkspaceActionsMenu(event) {
    if (event.target.id !== "ensoWorkspaceMoreActions") {
      return;
    }
    ...
    this.#contextMenuData = {
      workspaceId: target?.getAttribute("enso-workspace-id"),
      originalTarget: target,
    };
    const workspaceName = document.getElementById("context_ensoEditWorkspace");
    const themePicker = document.getElementById("context_ensoChangeWorkspaceTheme");
    /* We can't show the rename input properly in collapsed state,
    so hide the workspace edit input */
    const isCollapsed = !Services.prefs.getBoolPref("enso.view.sidebar-expanded");
    ...
    themePicker.hidden =
      this.#contextMenuData.workspaceId &&
      this.#contextMenuData.workspaceId !== this.activeWorkspace;
    const separator = document.getElementById("context_ensoWorkspacesSeparator");
    for (const item of event.target.querySelectorAll(".enso-workspace-context-menu-item")) {
      item.remove();
    }
    if (!this.#contextMenuData.workspaceId) {
      ...
      for (const workspace of this.getWorkspaces().reverse()) {
        ...
        item.addEventListener("command", (e) => {
          this.changeWorkspaceWithID(e.target.closest("menuitem").getAttribute("enso-workspace-id"));
        });
  ...
  #propagateWorkspaceData(aSpaceData = null) {
    ...
    window.gEnsoWindowSync.propagateWorkspacesToAllWindows(aSpaceData ?? this._workspaceCache);
  }
  ...
  async openWorkspaceCreation() {
    ...
    document.documentElement.setAttribute("enso-creating-workspace", "true");
    await this.createAndSaveWorkspace("Space", undefined, false, 0, {
      beforeChangeCallback: async (workspace) => {
        createForm = document.createXULElement("enso-workspace-creation");
  ...
  #unpinnedTabsInWorkspace(workspaceID) {
    return Array.from(this.allStoredTabs).filter(
      (tab) => tab.getAttribute("enso-workspace-id") === workspaceID && tab.visible && !tab.pinned
    );
  }
  ...
  async unloadWorkspace() {
    ...
    const tabsToUnload = this.allStoredTabs.filter(
      (tab) =>
        tab.getAttribute("enso-workspace-id") === workspaceId &&
        !tab.hasAttribute("enso-empty-tab") &&
        !tab.hasAttribute("enso-essential") &&
    ...
    for (let tab of tabs) {
      ...
      if (container) {
        if (tab.group?.hasAttribute("split-view-group")) {
          gBrowser.ensoHandleTabMove(tab.group, () => {
            for (const subTab of tab.group.tabs) {
              subTab.setAttribute("enso-workspace-id", workspaceID);
            }
            container.insertBefore(tab.group, container.lastChild);
          });
          continue;
        }
        gBrowser.ensoHandleTabMove(tab, () => {
      ...
      const glanceTab = tab.querySelector(".tabbrowser-tab[enso-glance-tab]");
      if (glanceTab) {
        glanceTab.setAttribute("enso-workspace-id", workspaceID);
      }
    }
    return true;
  }
  ...
  #prepareNewWorkspace(space) {
    document.documentElement.setAttribute("enso-workspace-id", space.uuid);
    let tabCount = 0;
    for (let tab of gBrowser.tabs) {
      const isEssential = tab.getAttribute("enso-essential") === "true";
      if (!tab.hasAttribute("enso-workspace-id") && !tab.pinned && !isEssential) {
  ...
  async changeWorkspace(workspace, ...args) {
    ...
    try {
      ...
      console.error("gEnsoWorkspaces: Error changing workspace", e);
    }
  ...
  makeSureEmptyTabIsFirst() {
    ...
    if (emptyTab) {
      emptyTab.setAttribute("enso-workspace-id", this.activeWorkspace);
  ...
  #fixTabPositions() {
    ...
    for (const tab of tabs) {
      recurseFolder(tab);
      tab._tPos = tPos++;
      if (!tab.hasAttribute("enso-empty-tab")) {
        tab._pPos = pPos++;
      }
    }
  }
  ...
  _organizeWorkspaceStripLocations(workspace, justMove = false, offsetPixels = 0) {
    ...
    const otherContainersEssentials = document.querySelectorAll(
      `#enso-essentials .enso-workspace-tabs-section`
    );
    ...
    if (offsetPixels) {
      ...
      if (nextWorkspace) {
        ...
        } = gEnsoThemePicker.getGradientForWorkspace(nextWorkspace);
        const existingGrain = gEnsoThemePicker.getGradientForWorkspace(workspace).grain;
        const percentage = Math.abs(offsetPixels) / 200;
        document.documentElement.style.setProperty("--enso-background-opacity", 1 - percentage);
        if (!this._hasAnimatedBackgrounds) {
          this._hasAnimatedBackgrounds = true;
          document.documentElement.style.setProperty(
            "--enso-main-browser-background-old",
            nextGradient
          );
          document.documentElement.style.setProperty(
            "--enso-main-browser-background-toolbar-old",
            nextToolbarGradient
          );
        ...
        if (!this.#inChangingWorkspace) {
          gEnsoThemePicker.updateNoise(grainValue);
        }
      }
    } else {
      delete this._hasAnimatedBackgrounds;
  ...
  updateWorkspaceIndicator(currentWorkspace, workspaceIndicator) {
    ...
    const indicatorName = workspaceIndicator.querySelector(".enso-current-workspace-indicator-name");
    const indicatorIcon = workspaceIndicator.querySelector(".enso-current-workspace-indicator-icon");
    const iconStack = workspaceIndicator.querySelector(".enso-current-workspace-indicator-stack");
  ...
  async _animateTabs(
    ...
    gEnsoUIManager.tabsWrapper.style.scrollbarWidth = "none";
    ...
    if (shouldAnimate && previousWorkspace) {
      ...
      gEnsoThemePicker.previousBackgroundOpacity = previousBackgroundOpacity;
      document.documentElement.style.setProperty(
        "--enso-background-opacity",
      ...
      animations.push(
        gEnsoUIManager.motion.animate(
          ...
          {
            "--enso-background-opacity": [previousBackgroundOpacity, 1],
          },
          {
            type: "spring",
            bounce: 0,
    ...
    for (const element of document.querySelectorAll("enso-workspace")) {
      if (element.classList.contains("enso-essentials-container")) {
        continue;
      }
    ...
    if (shouldAnimate) {
      gEnsoUIManager._preventToolbarRebuild = true;
  ...
  _shouldShowTab(tab, workspaceUuid, containerId, workspaces) {
    const isEssential = tab.getAttribute("enso-essential") === "true";
    const tabWorkspaceId = tab.getAttribute("enso-workspace-id");
    const tabContextId = tab.getAttribute("usercontextid") ?? "0";

    if (tab.hasAttribute("enso-glance-tab")) {
  ...
  async _handleTabSelection(workspace, onInit, previousWorkspaceId) {
    ...
    if (oldWorkspaceId && oldWorkspaceId !== workspace.uuid) {
      this.lastSelectedWorkspaceTabs[oldWorkspaceId] =
        gEnsoGlanceManager.getTabOrGlanceParent(currentSelectedTab);
    }
  ...
  async _updateWorkspaceState(
    ...
    document.documentElement.setAttribute("enso-workspace-id", workspace.uuid);
    ...
    // Update workspace UI
    requestAnimationFrame(() => {
      gEnsoThemePicker.onWorkspaceChange(workspace);
    });

    gEnsoUIManager.tabsWrapper.scrollbarWidth = "none";
    ...
    gEnsoUIManager.tabsWrapper.style.scrollbarWidth = "";
    ...
    if (onInit) {
      ...
      for (const tab of gBrowser.tabs) {
        if (!tab.hasAttribute("enso-workspace-id") && !tab.hasAttribute("enso-workspace-id")) {
          tab.setAttribute("enso-workspace-id", workspace.uuid);
        }
      }
      this.#fireSpaceUIUpdate();
    }
  ...
  #fireSpaceUIUpdate() {
    window.dispatchEvent(
      new CustomEvent("EnsoWorkspacesUIUpdate", {
        bubbles: true,
  ...
  updateWorkspacesChangeContextMenu() {
    ...
    const workspaces = this.getWorkspaces();
    const ctxCommand = document.getElementById("cmd_ensoCtxDeleteWorkspace");
    ...
    let menubar = document.getElementById("enso-spaces-menubar");
    ...
    for (const popup of itemsToFill) {
      ...
      for (const item of popup.querySelectorAll(".enso-workspace-context-menu-item")) {
        item.remove();
      }
      const separator = document.createXULElement("menuseparator");
      separator.classList.add("enso-workspace-context-menu-item");
      if (isMoveTabPopup) {
      ...
      for (let workspace of isMoveTabPopup ? workspaces.reverse() : workspaces) {
        ...
        if (isMoveTabPopup) {
          ...
          menuItem.setAttribute("command", "cmd_ensoChangeWorkspaceTab");
        } else {
          if (i < 10) {
            menuItem.setAttribute("key", `enso-workspace-switch-${i + 1}`);
          }
  ...
  #createWorkspaceData(name, icon, containerTabId = 0) {
    if (!this.currentWindowIsSyncing) {
      ...
      if (this.isPrivateWindow) {
        icon = gEnsoEmojiPicker.getSVGURL("eye.svg");
      }
    }
    let workspace = {
      uuid: gEnsoUIManager.generateUuidv4(),
      icon,
      name,
      theme: nsEnsoThemePicker.getTheme([]),
      containerTabId,
    };
    return workspace;
  }
  ...
  async createAndSaveWorkspace(
    ...
    const extraTabs = Array.from(gBrowser.tabContainer.arrowScrollbox.children).filter(
      (child) =>
        ...
        !child.hasAttribute("enso-workspace-id") &&
        !child.hasAttribute("enso-empty-tab") &&
        !child.hasAttribute("enso-essential")
    );
  ...
  updateShouldHideSeparator(arrowScrollbox, pinnedContainer, fromTabSelection = false) {
    ...
    if (shouldHideSeparator) {
      ...
      const workspaceID = pinnedContainer.getAttribute("enso-workspace-id");
      const tabs = this.#unpinnedTabsInWorkspace(workspaceID);
      const closableTabs = this.#getClosableTabs(tabs);
      const button = pinnedContainer.querySelector(".enso-workspace-close-unpinned-tabs-button");
  ...
  onPinnedTabsResize(entries, forAnimation = false) {
    ...
    for (const entry of entries) {
      let originalWorkspaceId = entry.target.getAttribute("enso-workspace-id");
      if (!originalWorkspaceId) {
        originalWorkspaceId = entry.target.closest("enso-workspace")?.id || this.activeWorkspace;
      }
      const workspacesIds = [];
      if (entry.target.closest("#enso-essentials")) {
  ...
  async onTabBrowserInserted(event) {
    ...
    if (workspaceID) {
      if (tab.hasAttribute("change-workspace") && this.moveTabToWorkspace(tab, workspaceID)) {
        this.lastSelectedWorkspaceTabs[workspaceID] = gEnsoGlanceManager.getTabOrGlanceParent(tab);
    ...
    if (tab.hasAttribute("enso-workspace-id")) {
      const tabWorkspaceId = tab.getAttribute("enso-workspace-id");
      this.moveTabToWorkspace(tab, tabWorkspaceId);
      await this.changeWorkspaceWithID(tabWorkspaceId);
    } else {
      tab.setAttribute("enso-workspace-id", activeWorkspace.uuid);
    }
  }

  #changeToEmptyTab() {
    const isEmpty = gBrowser.selectedTab.hasAttribute("enso-empty-tab");
    gEnsoCompactModeManager.sidebar.toggleAttribute("enso-has-empty-tab", isEmpty);
    document.documentElement.setAttribute("enso-has-empty-tab", isEmpty);
  }
  ...
  async onLocationChange(event) {
    ...
    if (tab.hasAttribute("enso-glance-tab")) {
      ...
      console.assert(tab, "Tab not found for enso-glance-tab");
    }
    const workspaceID = tab.getAttribute("enso-workspace-id");
    const isEssential = tab.getAttribute("enso-essential") === "true";
    if (tab.hasAttribute("enso-empty-tab")) {
  ...
  async closeAllUnpinnedTabs() {
    ...
    const restoreClosedTabsShortcut = gEnsoKeyboardShortcutsManager.getShortcutDisplayFromCommand(
      "History:RestoreLastClosedTabOrWindowOrSession"
    );

    gEnsoUIManager.showToast("enso-workspaces-close-all-unpinned-tabs-toast", {
      l10nArgs: {
  ...
  async contextDeleteWorkspace() {
    ...
    const [title, body] = await document.l10n.formatValues([
      { id: "enso-workspaces-delete-workspace-title" },
      {
        id: "enso-workspaces-delete-workspace-body",
  ...
  #initializeWorkspaceTabContextMenus() {
    if (this.privateWindowOrDisabled) {
      const commandsToDisable = [
        ...
        "enso-context-menu-new-folder-toolbar",
      ];
  ...
  async changeTabWorkspace(workspaceID) {
    ...
    const previousWorkspaceID = document.documentElement.getAttribute("enso-workspace-id");
    for (let tab of tabs) {
    ...
    this.lastSelectedWorkspaceTabs[workspaceID] = gEnsoGlanceManager.getTabOrGlanceParent(
      tabs[tabs.length - 1]
    );
    const workspaces = this.getWorkspaces();
  ...
  getTabsToExclude(aTab) {
    const tabWorkspaceId = aTab.getAttribute("enso-workspace-id");
  ...
  get allTabGroups() {
    ...
    for (const container of containers) {
      ...
      for (const tabGroup of container.querySelectorAll("enso-folder")) {
        tabGroups.push(tabGroup);
      }
    }
    return tabGroups;
  ...
  async switchTabIfNeeded(tab) {
    ...
    try {
      ...
      if (
        (tab.getAttribute("enso-workspace-id") !== this.activeWorkspace &&
          !tab.hasAttribute("enso-essential")) ||
        ...
        if (tab.hasAttribute("enso-essential")) {
          ...
          workspaceToSwitch = this.getWorkspaceFromId(tab.getAttribute("enso-workspace-id"));
        }
        ...
        try {
          this.lastSelectedWorkspaceTabs[workspaceToSwitch.uuid] =
            gEnsoGlanceManager.getTabOrGlanceParent(tab);
          await this.changeWorkspace(workspaceToSwitch);
        } finally {
          this._workspaceChangeInProgress = false;
        }
  ...
  onWindowResize(event = undefined) {
    ...
    if (!gEnsoPinnedTabManager.expandedSidebarMode) {
  ...
  handleTabCloseWindow() {
    if (Services.prefs.getBoolPref("enso.tabs.close-window-with-empty")) {
      document.getElementById("cmd_closeWindow").doCommand();
    }
  }
}

window.gEnsoWorkspaces = new nsEnsoWorkspaces();
