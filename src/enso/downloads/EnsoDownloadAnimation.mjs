...
import {
  nsEnsoDOMOperatedFeature,
  nsZenMultiWindowFeature,
} from "chrome://browser/content/enso-components/EnsoCommonUtils.mjs";
...
class nsEnsoDownloadAnimation extends nsEnsoDOMOperatedFeature {
  async init() {
  ...
  async #setupDownloadListeners() {
    try {
      ...
      console.error(
        `[${nsEnsoDownloadAnimation.name}] Failed to set up download animation listeners: ${error}`
      );
    }
  }
  ...
  #handleNewDownload() {
    if (
      !Services.prefs.getBoolPref("enso.downloads.download-animation") ||
      !nsZenMultiWindowFeature.isActiveWindow
    ) {
      return;
    }

    if (!gEnsoUIManager._lastClickPosition) {
      console.warn(
        `[${nsEnsoDownloadAnimation.name}] No recent click position available for animation`
      );
      return;
    }

    this.#animateDownload(gEnsoUIManager._lastClickPosition);
  }

  #animateDownload(startPosition) {
    let animationElement = document.querySelector("enso-download-animation");

    if (!animationElement) {
      animationElement = document.createElement("enso-download-animation");
...
class nsEnsoDownloadAnimationElement extends HTMLElement {
  ...
  #loadArcStyles() {
    try {
      ...
      link.setAttribute(
        "href",
        "chrome://browser/content/enso-styles/enso-download-arc-animation.css"
      );
      this.shadowRoot.appendChild(link);
    } catch (error) {
      console.error(`[${nsEnsoDownloadAnimationElement.name}] Error loading arc styles: ${error}`);
    }
  }
  ...
  async initializeAnimation(startPosition) {
    if (!startPosition) {
      console.warn(
        `[${nsEnsoDownloadAnimationElement.name}] No start position provided, skipping animation`
      );
      return;
    }
  ...
  #areTabsOnRightSide() {
    const position = Services.prefs.getIntPref("enso.downloads.icon-popup-position", 0);
    if (position === 1) {
      return false;
    }
    if (position === 2) {
    ...
    return Services.prefs.getBoolPref("enso.tabs.vertical.right-side");
  }
  ...
  #determineEndPosition() {
    ...
    if (isDownloadButtonVisible) {
      ...
      const areTabsPositionedRight = this.#areTabsOnRightSide();
      const wrapper = document.getElementById("enso-main-app-wrapper");
  ...
  #createArcAnimationElement(startPosition) {
    const arcAnimationHTML = `
            <box class="enso-download-arc-animation">
              <box class="enso-download-arc-animation-inner-circle">
                <html:div class="enso-download-arc-animation-icon"></html:div>
              </box>
            </box>
          `;

    const fragment = window.MozXULElement.parseXULToFragment(arcAnimationHTML);
    const animationElement = fragment.querySelector(".enso-download-arc-animation");
  ...
  async #startDownloadAnimation(
    ...
    try {
      ...
      await gEnsoUIManager.motion.animate(arcAnimationElement, sequence, {
        duration: Services.prefs.getIntPref("enso.downloads.download-animation-duration") / 1000,
      ...
      console.error("[nsEnsoDownloadAnimationElement] Error in animation sequence:", error);
      this.#cleanArcAnimation(arcAnimationElement);
    }
  }
  ...
  async #startBoxAnimation(areTabsPositionedRight) {
    ...
    if (this.#isBoxAnimationRunning) {
      console.warn(
        `[${nsEnsoDownloadAnimationElement.name}] Box animation already running, skipping new request.`
      );
      return;
    }
    ...
    if (!wrapper) {
      console.warn(
        `[${nsEnsoDownloadAnimationElement.name}] Cannot start box animation, Wrapper element not found`
      );
      return;
    }
    ...
    try {
      const boxAnimationHTML = `
            <box class="enso-download-box-animation">
              <html:div class="enso-download-box-animation-icon"></html:div>
            </box>
          `;
      ...
      this.#boxAnimationElement = fragment.querySelector(".enso-download-box-animation");
      ...
      console.error(
        `[${nsEnsoDownloadAnimationElement.name}] Error during box entry animation: ${error}`
      );
  ...
  #getBoxAnimationDurationMs() {
    return Services.prefs.getIntPref("enso.downloads.download-animation-duration") + 200;
  }
  ...
  async #finishBoxAnimation(areTabsPositionedRight) {
    ...
    try {
      ...
      console.warn(
        `[${nsEnsoDownloadAnimationElement.name}] Error during box exit animation: ${error}`
      );
    } finally {
      this.#cleanBoxAnimation();
    }
  ...
  #cleanBoxAnimation() {
    if (this.#boxAnimationElement && this.#boxAnimationElement.isConnected) {
      try {
        ...
        console.error(
          `[${nsEnsoDownloadAnimationElement.name}] Error removing box animation element: ${error}`,
          error
        );
      }
    }
...
customElements.define("enso-download-animation", nsEnsoDownloadAnimationElement);

new nsEnsoDownloadAnimation();
