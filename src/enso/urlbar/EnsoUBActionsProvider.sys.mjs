...
import { globalActions } from "resource:///modules/EnsoUBGlobalActions.sys.mjs";
import { ensoUrlbarResultsLearner } from "./EnsoUBResultsLearner.sys.mjs";

const lazy = {};

const DYNAMIC_TYPE_NAME = "enso-actions";
...
XPCOMUtils.defineLazyPreferenceGetter(
  lazy,
  "enabledPref",
  "enso.urlbar.suggestions.quick-actions",
  true
);
...
export class ZenUrlbarProviderGlobalActions extends UrlbarProvider {
  ...
  async isActive(queryContext) {
    return (
      queryContext.searchMode?.source == UrlbarUtils.RESULT_SOURCE.ENSO_ACTIONS ||
      (lazy.enabledPref &&
  ...
  #getWorkspaceActions(window) {
    if (window.gEnsoWorkspaces.privateWindowOrDisabled) {
      return [];
    }
    const workspaces = window.gEnsoWorkspaces.getWorkspaces();
    if (!workspaces?.length) {
      return [];
    }
    let actions = [];
    const activeSpaceUUID = window.gEnsoWorkspaces.activeWorkspace;
    for (const workspace of workspaces) {
      if (workspace.uuid !== activeSpaceUUID) {
        const accentColor = window.gEnsoWorkspaces
          .workspaceElement(workspace.uuid)
          ?.style.getPropertyValue("--enso-primary-color");
        actions.push({
          label: "Focus on",
          extraPayload: {
          ...
          commandId: `enso:workspace-${workspace.uuid}`,
  ...
  async #getExtensionActions(window) {
    ...
    if (window.gBrowser.selectedTab.hasAttribute("enso-empty-tab")) {
    ...
    return addons
      ...
      .map((addon) => {
        return {
          ...
          label: "Extension",
          commandId: `enso:extension-${addon.id}`,
          extraPayload: {
  ...
  async startQuery(queryContext, addCallback) {
    ...
    const isPrefixed = queryContext.searchMode?.source == UrlbarUtils.RESULT_SOURCE.ENSO_ACTIONS;
    if (!query && !isPrefixed) {
      return;
    }
    ...
    for (const action of actionsResults) {
      const { payload, payloadHighlights } = lazy.UrlbarResult.payloadAndSimpleHighlights([], {
        ...
        title: action.label,
        ensoCommand: action.command,
        dynamicType: DYNAMIC_TYPE_NAME,
        ensoAction: true,
        query: isPrefixed ? action.label.trimStart() : queryContext.searchString,
        icon: action.icon,
        shortcutContent: ownerGlobal.gEnsoKeyboardShortcutsManager.getShortcutDisplayFromCommand(
          action.command
        ),
      ...
      const shouldBePrioritized =
        ensoUrlbarResultsLearner.shouldPrioritize(action.commandId) && !isPrefixed;
      let result = new lazy.UrlbarResult({
        ...
        suggestedIndex: !shouldBePrioritized
          ? ensoUrlbarResultsLearner.getDeprioritizeIndex(action.commandId)
          : undefined,
      });
    ...
    let i = 0;
    ensoUrlbarResultsLearner.sortCommandsByPriority(finalResults).forEach((result) => {
    ...
    return {
      ...
      prettyName: {
        attributes: {
          ...
          style: `--enso-primary-color: ${result.payload.accentColor || "currentColor"}`,
        },
      },
      prettyNameTitle: {
  ...
  onSearchSessionEnd(_queryContext, _controller, details) {
    ...
    if (details?.result) {
      ...
      ensoUrlbarResultsLearner.recordExecution(usedCommand, [...this.#seenCommands]);
    }
  ...
  onEngagement(queryContext, controller, details) {
    ...
    const command = payload.ensoCommand;
    ...
    if (payload.workspaceId) {
      ownerGlobal.gEnsoWorkspaces.changeWorkspaceWithID(payload.workspaceId);
      return;
    }
