...
const ADDONS_BUTTONS_HIDDEN = Services.prefs.getBoolPref(
  "enso.theme.hide-unified-extensions-button",
  true
);
...
export class nsEnsoSiteDataPanel {
  #iconMap = {
    install: "extension",
  ...
  #init() {
    ...
    const button = this.window.MozXULElement.parseXULToFragment(`
      <box id="enso-site-data-icon-button" role="button" align="center" class="identity-box-button" delegatesanchor="true">
        <image />
      </box>
    `);
    this.anchor = button.querySelector("#enso-site-data-icon-button");
    ...
    this.document
      ...
      .setAttribute("addon-webext-overflowbutton", "enso-site-data-icon-button");
  ...
  #initEventListeners() {
    ...
    this.document.getElementById("enso-site-data-manage-addons").addEventListener("click", this);
    this.document.getElementById("enso-site-data-settings-more").addEventListener("click", this);
    this.anchor.addEventListener("click", this);
    const kCommandIDs = [
      "enso-site-data-header-share",
      "enso-site-data-header-bookmark",
      "enso-site-data-security-info",
      "enso-site-data-actions",
      "enso-site-data-new-addon-button",
    ];
  ...
  #initCopyUrlButton() {
    ...
    const fragment = this.window.MozXULElement.parseXULToFragment(`
      <hbox id="enso-copy-url-button"
            class="urlbar-page-action"
            role="button"
            data-l10n-id="enso-urlbar-copy-url-button"
            disabled="true">
    ...
    const aElement = this.document.getElementById("enso-copy-url-button");
    aElement.addEventListener("click", () => {
      if (aElement.hasAttribute("disabled")) {
        return;
      }
      this.document.getElementById("cmd_ensoCopyCurrentURL").doCommand();
    });
  ...
  #initContextMenuEventListener() {
    const kCommands = {
      context_ensoClearSiteData: (event) => {
        this.window.gIdentityHandler.clearSiteData(event);
      },
      context_ensoOpenGetAddons: () => {
        this.#openGetAddons();
      },
      context_ensoOpenSiteSettings: () => {
  ...
  #initUnifiedPanel() {
    const panel = this.document.getElementById("enso-unified-site-data-panel");
  ...
  #setAddonsOverflow() {
    const addons = this.document.getElementById("enso-site-data-addons");
  ...
  #setSiteHeader() {
    {
      const button = this.document.getElementById("enso-site-data-header-reader-mode");
    ...
    {
      const button = this.document.getElementById("enso-site-data-header-bookmark");
    ...
    {
      const button = this.document.getElementById("enso-site-data-header-share");
  ...
  #setSiteSecurityInfo() {
    ...
    const button = this.document.getElementById("enso-site-data-security-info");
    ...
    if (gIdentityHandler._pageExtensionPolicy) {
      this.document.l10n.setAttributes(button, "enso-site-data-security-info-extension");
      identity = "extension";
    } else if (
      ...
      this.document.l10n.setAttributes(button, "enso-site-data-security-info-secure");
      identity = "secure";
    } else {
      this.document.l10n.setAttributes(button, "enso-site-data-security-info-not-secure");
      identity = "not-secure";
    }
  ...
  #setSitePermissions() {
    ...
    const list = this.document.getElementById("enso-site-data-settings-list");
    const section = list.closest(".enso-site-data-section");
    ...
    switch (permission.state) {
      ...
      case SitePermissions.ALLOW:
        if (permission.id === "site-protection") {
          return "enso-site-data-protections-enabled";
        }
        return "enso-site-data-setting-allow";
      case SitePermissions.BLOCK:
      case SitePermissions.AUTOPLAY_BLOCKED_ALL:
        if (permission.id === "site-protection") {
          return "enso-site-data-protections-disabled";
        }
        return "enso-site-data-setting-block";
      default:
        return null;
    }
  }
  ...
  #createPermissionItem(id, key, permission) {
    ...
    img.classList.add("permission-popup-permission-icon", "enso-site-data-permission-icon");
    img.setAttribute("closemenu", "none");
    if (this.#iconMap[id]) {
      img.classList.add(`enso-permission-${this.#iconMap[id]}-icon`);
    }
    ...
    if (isCrossSiteCookie) {
      this.document.l10n.setAttributes(nameLabel, "enso-site-data-setting-cross-site");
    } else {
      let label = SitePermissions.getPermissionLabel(permission.id);
      if (label) {
        nameLabel.textContent = label;
      } else {
        this.document.l10n.setAttributes(nameLabel, "enso-site-data-setting-" + idNoSuffix);
      }
    }
    labelContainer.appendChild(nameLabel);

    let stateLabel = this.document.createXULElement("label");
    stateLabel.setAttribute("class", "enso-permission-popup-permission-state-label");
    if (isCrossSiteCookie) {
  ...
  #onCommandEvent(event) {
    ...
    switch (id) {
      case "enso-site-data-new-addon-button": {
        this.#openGetAddons();
        break;
      }
      case "enso-site-data-security-info": {
        this.window.gIdentityHandler._openPopup(event);
        break;
      }
      case "enso-site-data-actions": {
        const button = this.document.getElementById("enso-site-data-actions");
        const popup = this.document.getElementById("ensoSiteDataActions");
        popup.openPopup(
          button,
          "after_start",
          0,
      ...
      case "enso-site-data-header-bookmark": {
        this.window.BookmarkingUI.onStarCommand(event);
        break;
      }
      case "enso-site-data-header-share": {
        /* eslint-disable mozilla/valid-services */
        if (Services.enso.canShare()) {
          ...
          this.window.gEnsoCommonActions.copyCurrentURLToClipboard();
        }
  ...
  #onPermissionClick(label) {
    ...
    if (!isCrossSiteCookie) {
      label
        .querySelector(".enso-permission-popup-permission-state-label")
  ...
  #onClickEvent(event) {
    const id = event.target.id;
    switch (id) {
      case "enso-site-data-manage-addons": {
        const { BrowserAddonUI } = this.window;
        BrowserAddonUI.openAddonsMgr("addons://list/extension");
        this.unifiedPanel.hidePopup();
        break;
      }
      case "enso-site-data-settings-more": {
        const { BrowserCommands } = this.window;
        BrowserCommands.pageInfo(null, "permTab");
        break;
      }
      case "enso-site-data-icon-button": {
  ...
  async #maybeShowFeatureCallout() {
    const kPref = "enso.site-data-panel.show-callout";
    if (!Services.prefs.getBoolPref(kPref, false)) {
      return;
    }
    Services.prefs.setBoolPref(kPref, false);
    const { gBrowser, gEnsoWorkspaces } = this.window;
    ...
    this.window.setTimeout(() => {
      callout.showFeatureCallout({
        id: "ENSO_EXTENSIONS_PANEL_MOVE_CALLOUT",
          ...
          screens: [
            {
              ...
              anchors: [
                {
                  selector: "#enso-site-data-icon-button",
                  panel_position: {
              ...
              content: {
                ...
                width: "355px",
                title: {
                  string_id: "enso-site-data-panel-feature-callout-title",
                },
                subtitle: {
                  string_id: "enso-site-data-panel-feature-callout-subtitle",
                },
                dismiss_button: {
                  action: {
                    dismiss: true,
