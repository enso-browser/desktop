...
/**
 * Manages the Enso Glance feature - a preview overlay system for tabs
 * Allows users to preview content without fully opening new tabs
 */
class nsEnsoGlanceManager extends nsEnsoDOMOperatedFeature {
  // Animation state
  _animating = false;
  _lazyPref = {};
  ...
  #GLANCE_ANIMATION_DURATION = Services.prefs.getIntPref("enso.glance.animation-duration") / 1000;
  ...
  #insertIntoContextMenu() {
    ...
    menuitem.setAttribute("id", "context-ensoOpenLinkInGlance");
    menuitem.setAttribute("hidden", "true");
    menuitem.setAttribute("data-l10n-id", "enso-open-link-in-glance");
  ...
  handleMainCommandSet(event) {
    ...
    const commandHandlers = {
      cmd_ensoGlanceClose: () => this.closeGlance({ onTabClose: true }),
      cmd_ensoGlanceExpand: () => this.fullyOpenGlance(),
      cmd_ensoGlanceSplit: () => this.splitGlance(),
    };
  ...
  #createBrowserElement(url, currentTab, existingTab = null) {
    ...
    const newUUID = gEnsoUIManager.generateUuidv4();
  ...
  #configureNewTab(newTab, currentTab, glanceId) {
    if (currentTab.hasAttribute("ensoDefaultUserContextId")) {
      newTab.setAttribute("ensoDefaultUserContextId", true);
    }

    currentTab.querySelector(".tab-content").appendChild(newTab);
    newTab.setAttribute("enso-glance-tab", true);
  ...
  #createNewOverlayButtons() {
    const template = document.getElementById("enso-glance-sidebar-template");
    const newButtons = template.content.cloneNode(true);
    const container = newButtons.querySelector(".enso-glance-sidebar-container");
  ...
  #animateOverlayButtons(container) {
    ...
    const xOffset = gEnsoVerticalTabsManager._prefsRightSide ? 20 : -20;
  ...
  openGlance(data, existingTab = null, ownerTab = null) {
    ...
    this.overlay.classList.add("enso-glance-overlay");
  ...
  #animateGlanceOpening(data, browserElement) {
    ...
    return new Promise(async (resolve) => {
      ...
      gEnsoViewSplitter.onLocationChange(browserElement);
  ...
  #createGlancePreviewElement(src) {
    ...
    parent.classList.add("enso-glance-element-preview");
  ...
  #executeGlanceAnimation(data, browserElement, resolve) {
    ...
    gEnsoUIManager.motion
      .animate(this.browserWrapper, arcSequence, {
        duration: gEnsoUIManager.testingEnabled ? 0 : this.#GLANCE_ANIMATION_DURATION,
        ease: "easeInOut",
      })
      .then(() => {
  ...
  closeGlance({
    ...
    const sidebarButtons = this.browserWrapper.querySelector(".enso-glance-sidebar-container");
  ...
  #handleConfirmationTimeout(onTabClose, hasFocused, sidebarButtons) {
    if (onTabClose && hasFocused && !this.#confirmationTimeout && sidebarButtons) {
      const cancelButton = sidebarButtons.querySelector(".enso-glance-sidebar-close");
    ...
    return new Promise((resolve) => {
      ...
      gEnsoUIManager.motion
        ...
        .then(() => {
          ...
          const elementPreview = this.browserWrapper.querySelector(".enso-glance-element-preview");
          if (elementPreview) {
  ...
  #cleanupGlanceElements(lastCurrentTab) {
    this.overlay.classList.remove("enso-glance-overlay");
    gBrowser
      ._getSwitcher()
    ...
    if (
      ...
      this.#currentParentTab.linkedBrowser.ensoModeActive = false;
    }
    ...
    lastCurrentTab.removeAttribute("enso-glance-tab");
  ...
  #configureGlanceElements() {
    ...
    parentBrowserContainer.classList.add("enso-glance-background");
    parentBrowserContainer.classList.remove("enso-glance-overlay");
   ...
   */
  #setGlanceStates() {
    this.#currentParentTab.linkedBrowser.ensoModeActive = true;
    this.#currentParentTab.linkedBrowser.docShellIsActive = true;
    this.#currentBrowser.ensoModeActive = true;
    this.#currentBrowser.docShellIsActive = true;
    this.#currentBrowser.setAttribute("enso-glance-selected", true);
  ...
  #removeParentBackground(parentHasBrowser, browserContainer) {
    if (parentHasBrowser) {
      browserContainer.classList.remove("enso-glance-background");
    }
  }
  ...
  #resetGlanceStates(closeCurrentTab, closeParentTab, parentHasBrowser, browserContainer) {
    ...
    this.#currentBrowser.ensoModeActive = false;
    ...
    this.#currentBrowser.removeAttribute("enso-glance-selected");
    this.overlay.classList.remove("enso-glance-overlay");
  }
  ...
  shouldOpenTabInGlance(tab, uri) {
    ...
    return (
      ...
      Services.prefs.getBoolPref("enso.glance.enabled", true)
    );
  }
  ...
  async fullyOpenGlance({ forSplit = false } = {}) {
    ...
    this.#currentTab.setAttribute("enso-dont-split-glance", true);
    ...
    if (gReduceMotion) {
      gEnsoViewSplitter.deactivateCurrentSplitView();
      this.finishOpeningGlance();
      return;
    }
  ...
  #handleZenFolderPinning() {
    const isEnsoFolder = this.#currentParentTab?.group?.isEnsoFolder;
    if (Services.prefs.getBoolPref("enso.folders.owned-tabs-in-folder") && isEnsoFolder) {
  ...
  #prepareTabForFullOpen() {
    this.#currentTab.removeAttribute("enso-glance-tab");
    this.#clearContainerStyles(this.browserWrapper);
    ...
    this.#currentParentTab.linkedBrowser
      ...
      .classList.remove("enso-glance-background");
    this.#currentParentTab._visuallySelected = false;
  ...
  async #animateFullOpen(browserRect) {
    ...
    gEnsoViewSplitter.deactivateCurrentSplitView({ removeDeckSelected: true });
  }
  ...
  openGlanceForBookmark(event) {
    const activationMethod = Services.prefs.getStringPref("enso.glance.activation-method", "ctrl");
  ...
  async splitGlance() {
    ...
    gEnsoViewSplitter.splitTabs([currentTab, currentParentTab], "vsep", 1);

    const browserContainer = currentTab.linkedBrowser?.closest(".browserSidebarContainer");
    if (!gReduceMotion && browserContainer) {
      gEnsoViewSplitter.animateBrowserDrop(browserContainer);
    }
  }
  ...
  #handleZenFolderPinningForSplit(parentTab) {
    const isEnsoFolder = parentTab?.group?.isEnsoFolder;
    if (Services.prefs.getBoolPref("enso.folders.owned-tabs-in-folder") && isEnsoFolder) {
      gBrowser.pinTab(this.#currentTab);
    }
  }
  ...
  shouldShowDeckSelected(currentPanel, oldPanel) {
    ...
    if (currentGlanceID && oldGlanceID) {
      return (
        currentGlanceID === oldGlanceID && oldPanel.classList.contains("enso-glance-background")
      );
    }
  ...
  #isGlanceEnabledForSearch() {
    return (
      Services.prefs.getBoolPref("enso.glance.enabled", false) &&
      Services.prefs.getBoolPref("enso.glance.enable-contextmenu-search", true)
    );
  }
  ...
  #openGlanceForSearch(currentTab, parentTab) {
    ...
    const clickPosition = gEnsoUIManager._lastClickPosition || {
...
window.gEnsoGlanceManager = new nsEnsoGlanceManager();
