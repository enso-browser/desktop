...
XPCOMUtils.defineLazyPreferenceGetter(
  ...
  "enso.view.compact.toolbar-flash-popup.duration",
  800
);
...
ChromeUtils.defineLazyGetter(lazy, "mainAppWrapper", () =>
  document.getElementById("enso-main-app-wrapper")
);

window.gEnsoCompactModeManager = {
  _flashTimeouts: {},
  _eventListeners: [],
  _removeHoverFrames: {},

  // Delay to avoid flickering when hovering over the sidebar
  HOVER_HACK_DELAY: Services.prefs.getIntPref("enso.view.compact.hover-hack-delay", 0),

  preInit() {
    this._wasInCompactMode = Services.prefs.getBoolPref(
      "enso.view.compact.enable-at-startup",
      false
    );
    this._canDebugLog = Services.prefs.getBoolPref("enso.view.compact.debug", false);
  ...
  init() {
    ...
    Services.prefs.addObserver("enso.tabs.vertical.right-side", tabIsRightObserver);

    window.addEventListener(
      "unload",
      () => {
        Services.prefs.removeObserver("enso.tabs.vertical.right-side", tabIsRightObserver);
      },
      { once: true }
    );

    gEnsoUIManager.addPopupTrackingAttribute(this.sidebar);
    gEnsoUIManager.addPopupTrackingAttribute(
      document.getElementById("enso-appcontent-navbar-wrapper")
    );
    ...
    this._canShowBackgroundTabToast = Services.prefs.getBoolPref(
      "enso.view.compact.show-background-tab-toast",
      true
    );

    if (AppConstants.platform == "macosx") {
      window.addEventListener("mouseover", (event) => {
        const buttons = gEnsoVerticalTabsManager.actualWindowButtons;
  ...
  log(...args) {
    if (this._canDebugLog) {
      ...
      console.debug("[Enso Compact Mode]", ...args);
    }
  },

  get preference() {
    return document.documentElement.getAttribute("enso-compact-mode") === "true";
  },
  ...
  set preference(value) {
    ...
    if (
      ...
      document.documentElement.hasAttribute("enso-compact-animating")
    ) {
    ...
    this.sidebar.removeAttribute("enso-user-show");
    // We use this element in order to make it persis across restarts, by using the XULStore.
    // main-window can't store attributes other than window sizes, so we use this instead
    lazy.mainAppWrapper.setAttribute("enso-compact-mode", value);
    document.documentElement.setAttribute("enso-compact-mode", value);
    if (typeof this._wasInCompactMode === "undefined") {
      Services.prefs.setBoolPref("enso.view.compact.enable-at-startup", value);
    }
    this._updateEvent();
  },
  ...
  get sidebarIsOnRight() {
    ...
    this._sidebarIsOnRight = Services.prefs.getBoolPref("enso.tabs.vertical.right-side");
    return this._sidebarIsOnRight;
  },
  ...
  addHasPolyfillObserver() {
    const attributes = ["panelopen", "open", "breakout-extend", "enso-floating-urlbar"];
    this.sidebarObserverId = EnsoHasPolyfill.observeSelectorExistence(
      this.sidebar,
      [
        {
          selector:
            ":is([panelopen='true'], [open='true'], [breakout-extend='true']):not(#urlbar[enso-floating-urlbar='true']):not(tab):not(.enso-compact-mode-ignore)",
        },
      ],
      "enso-compact-mode-active",
      attributes
    );
    this.toolbarObserverId = EnsoHasPolyfill.observeSelectorExistence(
      document.getElementById("enso-appcontent-navbar-wrapper"),
      [
        {
          selector:
            ":is([panelopen='true'], [open='true'], #urlbar:focus-within, [breakout-extend='true']):not(.enso-compact-mode-ignore)",
        },
      ],
      "enso-compact-mode-active",
      attributes
    ...
    EnsoHasPolyfill.connectObserver(this.toolbarObserverId);
  },
  ...
  addContextMenu() {
    const fragment = window.MozXULElement.parseXULToFragment(`
      <menu id="enso-context-menu-compact-mode" data-l10n-id="enso-toolbar-context-compact-mode">
        <menupopup>
          <menuitem id="enso-context-menu-compact-mode-toggle" data-l10n-id="enso-toolbar-context-compact-mode-enable" type="checkbox" command="cmd_ensoCompactModeToggle"/>
          <menuseparator/>
          <menuitem id="enso-context-menu-compact-mode-hide-sidebar" data-l10n-id="enso-toolbar-context-compact-mode-just-tabs" type="radio" />
          <menuitem id="enso-context-menu-compact-mode-hide-toolbar" data-l10n-id="enso-toolbar-context-compact-mode-just-toolbar" type="radio" />
          <menuitem id="enso-context-menu-compact-mode-hide-both" data-l10n-id="enso-toolbar-context-compact-mode-hide-both" type="radio" />
        </menupopup>
      </menu>
    `);

    const idToAction = {
      "enso-context-menu-compact-mode-hide-sidebar": this.hideSidebar.bind(this),
      "enso-context-menu-compact-mode-hide-toolbar": this.hideToolbar.bind(this),
      "enso-context-menu-compact-mode-hide-both": this.hideBoth.bind(this),
    };
  ...
  updateCompactModeContext(isSingleToolbar) {
    ...
    const menuitem = document.getElementById("enso-context-menu-compact-mode-toggle");
    const menu = document.getElementById("enso-context-menu-compact-mode");
    if (!menu) {
      return;
    }
    if (isSingleToolbar) {
    ...
    const hideToolbarMenuItem = document.getElementById(
      "enso-context-menu-compact-mode-hide-toolbar"
    );
    if (isIllegalState) {
  ...
  hideSidebar() {
    Services.prefs.setBoolPref("enso.view.compact.hide-tabbar", true);
    Services.prefs.setBoolPref("enso.view.compact.hide-toolbar", false);
    this.callAllEventListeners();
  },

  hideToolbar() {
    Services.prefs.setBoolPref("enso.view.compact.hide-toolbar", true);
    Services.prefs.setBoolPref("enso.view.compact.hide-tabbar", false);
    this.callAllEventListeners();
  },
  ...
  checkIfIllegalState() {
    ...
    const isSidebarExpanded = gEnsoVerticalTabsManager._prefsSidebarExpanded;
    ...
    const isLeftSideButtons = !gEnsoVerticalTabsManager.isWindowsStyledButtons;
    const isRightSidebar = gEnsoVerticalTabsManager._prefsRightSide;
  ...
  async _updateEvent() {
    ...
    if (this.preference) {
      EnsoHasPolyfill.connectObserver(this.sidebarObserverId);
    } else {
      EnsoHasPolyfill.disconnectObserver(this.sidebarObserverId);
    }
    window.dispatchEvent(new CustomEvent("EnsoCompactMode:Toggled", { detail: this.preference }));
  },
  ...
  getAndApplySidebarWidth(event = undefined) {
    ...
    const shouldRecalculate =
      this.preference || document.documentElement.hasAttribute("enso-creating-workspace");
    const sidebarExpanded = document.documentElement.hasAttribute("enso-sidebar-expanded");
    if (sidebarWidth > 1) {
      ...
      this.sidebar.style.setProperty("--actual-enso-sidebar-width", `${sidebarWidth}px`);
      window.dispatchEvent(new window.Event("resize")); // To recalculate the layout
      if (
        event &&
        shouldRecalculate &&
        sidebarExpanded &&
        !gEnsoVerticalTabsManager._hadSidebarCollapse
      ) {
        return;
      }
      delete gEnsoVerticalTabsManager._hadSidebarCollapse;
      this.sidebar.style.setProperty("--enso-sidebar-width", `${sidebarWidth}px`);
    }
    return sidebarWidth;
  },
  ...
  get canHideSidebar() {
    return (
      Services.prefs.getBoolPref("enso.view.compact.hide-tabbar") ||
  ...
  get canHideToolbar() {
    return (
      Services.prefs.getBoolPref("enso.view.compact.hide-toolbar") &&
      !gEnsoVerticalTabsManager._hasSetSingleToolbar
    );
  },
  ...
  animateCompactMode() {
    ...
    document.documentElement.setAttribute("enso-compact-animating", "true");
    return new Promise((resolve) => {
      // We need to set the splitter width before hiding it
      let splitterWidth = document
        .getElementById("enso-sidebar-splitter")
      ...
      window.requestAnimationFrame(() => {
        ...
        const elementSeparation = EnsoThemeModifier.elementSeparation;
        if (!canAnimate) {
          this.sidebar.removeAttribute("animate");
          document.documentElement.removeAttribute("enso-compact-animating");
        ...
        if (document.documentElement.hasAttribute("enso-sidebar-expanded")) {
  ...
  updateContextMenu() {
    const toggle = document.getElementById("enso-context-menu-compact-mode-toggle");
    if (!toggle) {
      return;
    }
    ...
    const idName = "enso-context-menu-compact-mode-hide-";
  ...
  toggleSidebar() {
    this.sidebar.toggleAttribute("enso-user-show");
  },
  ...
  get hideAfterHoverDuration() {
    ...
    return Services.prefs.getIntPref("enso.view.compact.toolbar-hide-after-hover.duration");
  },
  ...
  get hoverableElements() {
    return [
      ...
      {
        element: document.getElementById("enso-appcontent-navbar-wrapper"),
        screenEdge: "top",
      },
      {
        element: gEnsoVerticalTabsManager.actualWindowButtons,
      },
    ];
  },
  ...
  flashSidebar(duration = lazy.COMPACT_MODE_FLASH_DURATION) {
    ...
    if (!tabPanels.matches("[enso-split-view='true']")) {
  ...
  _setElementExpandAttribute(element, value, attr = "enso-has-hover") {
    const kVerifiedAttributes = ["enso-has-hover", "has-popup-menu", "enso-compact-mode-active"];
    const isToolbar = element.id === "enso-appcontent-navbar-wrapper";
    if (value) {
      if (attr === "enso-has-hover" && element !== gEnsoVerticalTabsManager.actualWindowButtons) {
        element.setAttribute("enso-has-implicit-hover", "true");
      ...
      if (
        ...
        ((gEnsoVerticalTabsManager._hasSetSingleToolbar &&
          (element.hasAttribute("should-hide") ||
            document.documentElement.hasAttribute("enso-has-bookmarks"))) ||
          (this.preference &&
            Services.prefs.getBoolPref("enso.view.compact.hide-toolbar") &&
            !gEnsoVerticalTabsManager._hasSetSingleToolbar))
      ) {
      ...
      if (attr === "enso-has-hover") {
        element.removeAttribute("enso-has-implicit-hover");
      }
  ...
  addMouseActions() {
    gURLBar.addEventListener("mouseenter", (event) => {
      if (event.target.closest("#urlbar[enso-floating-urlbar]")) {
        window.requestAnimationFrame(() => {
          this._setElementExpandAttribute(gEnsoVerticalTabsManager.actualWindowButtons, false);
        });
    ...
    for (let i = 0; i < this.hoverableElements.length; i++) {
        ...
        setTimeout(() => {
          ...
          window.requestAnimationFrame(() => {
            if (
              ...
              target.hasAttribute("enso-has-hover")
            ) {
              return;
            }
            this._setElementExpandAttribute(target, true);
      ...
      const onLeave = (event) => {
        if (AppConstants.platform == "macosx") {
          const buttonRect = gEnsoVerticalTabsManager.actualWindowButtons.getBoundingClientRect();
        ...
        setTimeout(() => {
          ...
          if (
            event.explicitOriginalTarget?.closest?.("#urlbar[enso-floating-urlbar]") ||
            (document.documentElement.getAttribute("supress-primary-adjustment") === "true" &&
              gEnsoVerticalTabsManager._hasSetSingleToolbar) ||
            this._hasHoveredUrlbar ||
            this._ignoreNextHover ||
  ...
  _clearAllHoverStates() {
    ...
    for (let entry of this.hoverableElements) {
      const target = entry.element;
      if (target && !target.matches(":hover") && target.hasAttribute("enso-has-hover")) {
        this._setElementExpandAttribute(target, false);
        this.clearFlashTimeout("has-hover" + target.id);
      }
    }
  ...
  isSidebarPotentiallyOpen() {
    ...
    return (
      this.sidebar.hasAttribute("enso-user-show") ||
      this.sidebar.hasAttribute("enso-has-hover") ||
      this.sidebar.hasAttribute("enso-has-empty-tab")
    );
  },
  ...
  async _onTabOpen(tab, inBackground) {
    if (
      ...
      gEnsoUIManager.showToast("enso-background-tab-opened-toast", {
        button: {
          id: "enso-open-background-tab-button",
          command: () => {
...
document.addEventListener(
  ...
  () => {
    gEnsoCompactModeManager.preInit();
  },
  { once: true }
);
