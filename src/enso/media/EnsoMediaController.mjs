...
class nsEnsoMediaController {
  ...
  init() {
    if (!Services.prefs.getBoolPref("enso.mediacontrols.enabled", true)) {
      return;
    }

    this.mediaTitle = document.querySelector("#enso-media-title");
    this.mediaArtist = document.querySelector("#enso-media-artist");
    this.mediaControlBar = document.querySelector("#enso-media-controls-toolbar");
    this.mediaProgressBar = document.querySelector("#enso-media-progress-bar");
    this.mediaCurrentTime = document.querySelector("#enso-media-current-time");
    this.mediaDuration = document.querySelector("#enso-media-duration");
    this.mediaFocusButton = document.querySelector("#enso-media-focus-button");
    this.mediaProgressBarContainer = document.querySelector("#enso-media-progress-hbox");
  ...
  #initEventListeners() {
    this.mediaControlBar.addEventListener("mousedown", (event) => {
      if (event.target.closest(":is(toolbarbutton,#enso-media-progress-hbox)")) {
        return;
      }
      this.onMediaFocus();
    });
    ...
    this.mediaControlBar.addEventListener("command", (event) => {
      ...
      switch (button.id) {
        case "enso-media-pip-button":
          this.onMediaPip();
          break;
        case "enso-media-close-button":
          this.onControllerClose();
          break;
        case "enso-media-focus-button":
          this.onMediaFocus();
          break;
        case "enso-media-mute-button":
          this.onMediaMute();
          break;
        case "enso-media-previoustrack-button":
          this.onMediaPlayPrev();
          break;
        case "enso-media-nexttrack-button":
          this.onMediaPlayNext();
          break;
        case "enso-media-playpause-button":
          this.onMediaToggle();
          break;
        case "enso-media-mute-mic-button":
          this.onMicrophoneMuteToggle();
          break;
        case "enso-media-mute-camera-button":
  ...
  setupMediaControlUI(metadata, positionState) {
    ...
    for (const key of this.supportedKeys) {
      const button = this.mediaControlBar.querySelector(`#enso-media-${key}-button`);
  ...
  onMediaFocus() {
    ...
    if (this._currentMediaController) {
      ...
      if (tab) {
        window.gEnsoWorkspaces.switchTabIfNeeded(tab);
      }
    }
  }
...
window.gEnsoMediaController = new nsEnsoMediaController();
