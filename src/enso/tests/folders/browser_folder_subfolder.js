...
add_task(async function test_Create_Folder() {
  ...
  const subfolder = await gEnsoFolders.createFolder([tab], {
    renameFolder: false,
    label: "subfolder",
  });
  const parent = await gEnsoFolders.createFolder([tab2], {
    renameFolder: false,
    label: "parent",
  });
  ...
  Assert.equal(
    ...
    2,
    "Subfolder contains the tab and the empty tab created by Enso Folders"
  );
