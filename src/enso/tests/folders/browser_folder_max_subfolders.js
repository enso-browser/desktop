...
// Keep in sync with the default value for `enso.folders.max-subfolders`
const TEST_MAX_FOLDERS = 5;

add_task(async function test_Max_Subfolders() {
  const folder = await gEnsoFolders.createFolder([], {
    renameFolder: false,
  });

  const subfolderItem = document.getElementById("context_ensoFolderNewSubfolder");
  ...
  document.getElementById("ensoFolderActions").hidePopup();
