...
add_task(async function test_Welcome_Steps() {
  ...
  await new Promise((resolve) => {
    setTimeout(async () => {
      ...
      await EventUtils.synthesizeMouseAtCenter(
        document.getElementById("enso-welcome-start-button"),
        {}
      );
      setTimeout(() => {
        resolve();
  ...
  const welcomeContent = document.getElementById("enso-welcome-page-content");

  for (const button of document.querySelectorAll("#enso-welcome-page-sidebar-buttons button")) {
  ...
  await goNextWelcomePage("enso-welcome-skip-button");
  ...
  await goNextWelcomePage("enso-generic-next");
  ok(true, "Welcome Search Step Test Finished");

  await new Promise((resolve) => {
    setTimeout(async () => {
      const essentials = welcomeContent.querySelector(
        "#enso-welcome-initial-essentials-browser-sidebar-essentials"
      ).children;
      Assert.greater(
        essentials.length,
        3,
  ...
  await goNextWelcomePage("enso-welcome-start-browsing");
  ...
  await new Promise((resolve) => {
    setTimeout(async () => {
      Assert.greater(
        ...
        3,
        "There should be more than 3 Enso Essentials after the welcome process"
      );
      Assert.equal(
        gBrowser.tabs.filter((tab) => tab.pinned && !tab.hasAttribute("enso-essential")).length,
        3,
      ...
      Assert.equal(
        group.label,
        "enso basics",
        'The first tab group should be labeled "enso basics" after the welcome process'
      );
      for (const tab of gBrowser.tabs) {
        ...
        if (tab.pinned) {
          if (!tab.hasAttribute("enso-essential")) {
            ok(
              tab.hasAttribute("enso-workspace-id"),
              "Pinned tabs should have a enso-workspace-id attribute"
            );
