...
add_setup(async () => {
  ...
  gEnsoPinnedTabManager.addToEssentials(tabs.slice(0, 3));
  await BrowserTestUtils.waitForCondition(
    () => tabs.slice(0, 3).every((tab) => tab.hasAttribute("enso-essential")),
    "all essentials ready"
  );

  const essentialTabs = gBrowser.tabs.filter((tab) => tab.hasAttribute("enso-essential"));
  Assert.equal(essentialTabs.length, 3, "3 essential tabs created");

  const workspaceTabs = gBrowser.tabs.filter(
    (tab) => !tab.hasAttribute("enso-essential") && !tab.hasAttribute("enso-empty-tab")
  );
...
add_task(async function cycleTabsByAttribute() {
  await SpecialPowers.pushPrefEnv({
    set: [["enso.tabs.ctrl-tab.ignore-essential-tabs", true]],
  });
