...
add_task(async function test_Basic_Split_View_Glance() {
  await basicSplitNTabs(async () => {
    await openGlanceOnTab(window, async (glanceTab) => {
      ...
      ok(
        gBrowser.tabpanels.hasAttribute("enso-split-view"),
...
add_task(async function test_Basic_Split_View_Glance_Expand() {
  await basicSplitNTabs(async (tabs) => {
    await openGlanceOnTab(
      window,
      async (glanceTab) => {
        await gEnsoGlanceManager.fullyOpenGlance();
        ok(
          !glanceTab.hasAttribute("enso-glance-tab"),
          "The glance tab should not have the enso-glance-tab attribute after expanding"
        );
  ...
  await basicSplitNTabs(
    async () => {
      await openGlanceOnTab(window, () => {
        Assert.strictEqual(
          document.getElementById("cmd_ensoGlanceSplit").getAttribute("disabled"),
          "true",
...
add_task(async function test_Basic_Split_View_Glance_Split() {
  ...
  await openGlanceOnTab(
    ...
    async (glanceTab) => {
      ...
      document.getElementById("cmd_ensoGlanceSplit").doCommand();
      await waitForSplitPromise;
      ok(
        !glanceTab.hasAttribute("enso-glance-tab"),
        "The glance tab should not have the enso-glance-tab attribute after splitting"
      );
      ok(
        gBrowser.tabpanels.hasAttribute("enso-split-view"),
        "The split view should not have crashed with two tabs in it"
