...
async function withBookmarksShowing(aCallback) {
  await SpecialPowers.pushPrefEnv({
    set: [["enso.view.hide-window-controls", false]],
  });
...
add_task(async function test_workspace_bookmark() {
  ...
  await withBookmarksShowing(async () => {
    ...
    await gEnsoWorkspaces.changeWorkspaceWithID(firstWorkspace.uuid);
    ...
    await gEnsoWorkspaces.changeWorkspace(secondWorkspace);
    await new Promise((resolve) => setTimeout(resolve, 100));
    ...
    await gEnsoWorkspaces.changeWorkspace(firstWorkspace);
    await new Promise((resolve) => setTimeout(resolve, 100));
    ...
    await gEnsoWorkspaces.removeWorkspace(secondWorkspace.uuid);
    Assert.equal(
      gEnsoWorkspaces.getWorkspaces().workspaces.length,
      1,
