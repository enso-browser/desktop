...
add_task(async function test_Glance_Basic_Open() {
  ...
  await openGlanceOnTab(async (glanceTab) => {
    ...
    ok(
      ...
      "The glance tab should not have the enso-glance-tab attribute"
    );
    Assert.strictEqual(
      gBrowser.tabs.filter((tab) => tab.hasAttribute("enso-glance-tab")).length,
      0,
      "There should be no enso-glance-tab attribute on any tab"
    );
    Assert.greater(
      glanceTab._tPos,
      selectedTab._tPos,
...
add_task(async function test_Glance_New_From_essential() {
  ...
  await BrowserTestUtils.withNewTab({ gBrowser, url: "https://example.com/" }, async (browser) => {
    const selectedTab = gBrowser.selectedTab;
    gEnsoPinnedTabManager.addToEssentials(selectedTab);
    await openGlanceOnTab(async (glanceTab) => {
      await gEnsoGlanceManager.fullyOpenGlance();
