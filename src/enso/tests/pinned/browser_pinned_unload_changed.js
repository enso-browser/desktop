...
add_task(async function test_Unload_Changed_Pinned() {
  await SpecialPowers.pushPrefEnv({
    set: [["enso.pinned-tab-manager.close-shortcut-behavior", "reset-unload-switch"]],
  });
  ...
  await BrowserTestUtils.withNewTab({ gBrowser, url: "https://example.com/1" }, async (browser) => {
    ...
    setTimeout(() => {
      ok(
        tab.hasAttribute("enso-pinned-changed"),
        "The tab should have a enso-pinned-changed attribute after being pinned"
      );
      ...
      setTimeout(() => {
        ok(
          !tab.hasAttribute("enso-pinned-changed"),
          "The tab should not have a enso-pinned-changed attribute after being closed"
        );
