...
add_task(async function test_Restore_Pinned_Tab() {
  await BrowserTestUtils.withNewTab(
    ...
    async function (browser) {
      ...
      ok(
        tab.parentElement.closest(".enso-workspace-pinned-tabs-section"),
...
add_task(async function test_Restore_Essential_Tab() {
  await BrowserTestUtils.withNewTab(
    ...
    async function (browser) {
      ...
      SessionWindowUI.restoreLastClosedTabOrWindowOrSession(window);
      tab = gBrowser.selectedTab;
      ok(tab.hasAttribute("enso-essential"), "The tab should be marked as essential after restore");
      ok(
        tab.parentElement.closest(".enso-essentials-container"),
