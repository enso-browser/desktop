...
async function withNewSyncedWindow(action) {
  ...
  await win.gEnsoWorkspaces.promiseInitialized;
  await action(win);
...
async function runSyncAction(action, callback, type) {
  await new Promise((resolve) => {
    window.gEnsoWindowSync.addSyncHandler(async function handler(aEvent) {
      if (aEvent.type === type) {
        window.gEnsoWindowSync.removeSyncHandler(handler);
