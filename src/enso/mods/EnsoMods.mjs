 ...
 */
class nsEnsoMods extends nsZenPreloadedFeature {
  // private properties start
  #kZenStylesheetModHeader = "/* Zen Mods - Generated by EnsoMods.";
...
`;
  ...
  get #modsBackend() {
    if (!this.#_modsBackend) {
      this.#_modsBackend = Cc["@mozilla.org/enso/mods-backend;1"].getService(Ci.nsIEnsoModsBackend);
    }
    return this.#_modsBackend;
  }

  get #styleSheetPath() {
    return PathUtils.join(PathUtils.profileDir, "chrome", "enso-themes.css");
  }
  ...
  async #insertStylesheet() {
    try {
      ...
      console.warn("[EnsoMods]: Error rebuilding mods styles:", e);
    }
  }
  ...
  async #getEnabledMods() {
    if (Services.prefs.getBoolPref("enso.themes.disable-all", false)) {
      // eslint-disable-next-line no-console
      console.info("[EnsoMods]: Mods are disabled by user preference.");
      return [];
    }
    ...
    const message =
      modList !== ""
        ? `[EnsoMods]: Loading enabled Zen mods: ${modList}.`
        : "[EnsoMods]: No enabled Zen mods.";
    ...
    for (const { preferences, enabled } of modsWithPreferences) {
      ...
      for (const { type, property, defaultValue } of preferences) {
        ...
        try {
          ...
          console.warn(
            `[EnsoMods]: Setting default value for ${property} to ${defaultValue} (${typeof defaultValue})`
          );
          ...
          if (
            ...
            console.warn(
              `[EnsoMods]: Warning, invalid data type received (${typeof defaultValue}), skipping.`
            );
            continue;
          }
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (attempt >= maxRetries) {
          console.error("[EnsoMods]: Error downloading file after retries", url, e);
        } else {
          console.warn(
            `[EnsoMods]: Download failed (attempt ${attempt} of ${maxRetries}), retrying in ${retryDelayMs}ms...`,
            url,
            e
          );
  ...
  get updatePref() {
    return "enso.mods.updated-value-observer";
  }

  get modsRootPath() {
    return PathUtils.join(PathUtils.profileDir, "chrome", "enso-themes");
  }

  get modsDataFile() {
    return PathUtils.join(PathUtils.profileDir, "enso-themes.json");
  }
  ...
  async getMods() {
    ...
    try {
      ...
      Services.wm
        ...
        .gEnsoUIManager.showToast("enso-themes-corrupted", {
          timeout: 8000,
        });
    }
  ...
  async getModPreferences(mod) {
    ...
    try {
      ...
      console.error(`[EnsoMods]: Error reading mod preferences for ${mod.name}:`, e);
      return [];
    }
  }
  ...
  async init() {
    try {
      ...
      if (
        Services.prefs.getBoolPref("enso.themes.disable-all", false) ||
        Services.appinfo.inSafeMode
      ) {
        console.warn("[EnsoMods]: Mods disabled by user or in safe mode.");
        return;
      }
      ...
      console.error("[EnsoMods]: Error loading Zen Mods:", e);
    }

    Services.prefs.addObserver(this.updatePref, this.#rebuildModsStylesheet.bind(this));
    Services.prefs.addObserver("enso.themes.disable-all", this.#handleDisableMods.bind(this));
  }

  #setNewMilestoneIfNeeded() {
    const previousMilestone = Services.prefs.getStringPref("enso.mods.milestone", "");
    if (previousMilestone != Services.appinfo.version) {
      Services.prefs.setStringPref("enso.mods.milestone", Services.appinfo.version);
      Services.prefs.clearUserPref("enso.mods.last-update");
    }
  }

  #shouldAutoUpdate() {
    const daysBeforeUpdate = Services.prefs.getIntPref("enso.mods.auto-update-days");
    const lastUpdatedSec = Services.prefs.getIntPref("enso.mods.last-update", -1);
    const nowSec = Math.floor(Date.now() / 1000);
    const daysSinceUpdate = (nowSec - lastUpdatedSec) / (60 * 60 * 24);

    return (
      (Services.prefs.getBoolPref("enso.mods.auto-update", true) &&
  ...
  async checkForModsUpdates() {
      ...
      Object.values(mods).map(async (currentMod) => {
        try {
          ...
          if (
            ...
            console.warn(
              `[EnsoMods]: Mod update found for mod ${currentMod.name} (${currentMod.id}), current: ${currentMod.version}, new: ${possibleNewModVersion.version}`
            );
          ...
          console.error("[EnsoMods]: Error checking for mod updates", e);
    ...
    Services.prefs.setIntPref("enso.mods.last-update", Math.floor(Date.now() / 1000));
  ...
  async removeMod(modId, triggerUpdate = true) {
    ...
    console.warn(`[EnsoMods]: Removing mod ${modPath}`);
  ...
  async enableMod(modId) {
    ...
    console.warn(`[EnsoMods]: Enabling mod ${mod.name}`);
  ...
  async disableMod(modId) {
    ...
    console.warn(`[EnsoMods]: Disabling mod ${mod.name}`);
  ...
  async installMod(mod) {
    try {
      ...
      console.error("[EnsoMods]: Error installing mod", mod.id, e);
    }
  }
  ...
  async checkForModChanges() {
    ...
    for (const [modId, mod] of Object.entries(mods)) {
      try {
        ...
        console.error("[EnsoMods]: Error checking for mod changes", e);
      }
    }
  ...
  async requestMod(modId) {
    ...
    console.warn(`[EnsoMods]: Fetching mod ${modId} info from ${url}`);
    ...
    if (data.ok) {
      try {
        ...
        console.error(`[EnsoMods]: Error parsing mod ${modId} info:`, e);
      }
    } else {
      console.error(`[EnsoMods]: Error fetching mod ${modId} info:`, data.status);
    }
...
window.gEnsoMods = new nsEnsoMods();
