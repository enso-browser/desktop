...
if [ $(basename $PWD) != "enso-icons" ]; then
  echo "You need to be in the enso-icons directory to run this script"
  exit 1
fi
...
do_icons() {
  ...
  for filename in $system/*.svg; do
    ...
    echo "*  skin/classic/browser/enso-icons/$filename                      (../shared/enso-icons/$system/$filename) " >> jar.inc.mn
  done
...
do_common_icons() {
  for filename in common/*.svg; do
    ...
    add_header_to_file $filename
    filename=$(basename $filename)
    echo "*  skin/classic/browser/enso-icons/$filename                      (../shared/enso-icons/common/$filename) " >> jar.inc.mn
  done
  for filename in common/selectable/*.svg; do
    # remove the os/ prefix
    add_header_to_file $filename
    filename=$(basename $filename)
    echo "*  skin/classic/browser/enso-icons/selectable/$filename          (../shared/enso-icons/common/selectable/$filename) " >> jar.inc.mn
  done
}
...
echo "  skin/classic/browser/enso-icons/icons.css                      (../shared/enso-icons/icons.css) " >> jar.inc.mn
